...
\end{equation*}
where $\mc{A}^{I} _{a J} = A^{\alpha} _{a} (T_{\alpha})^{I} _{\ J}$ is the background $\mathrm{Spin}(d , 1)$ connection, so the action becomes
\begin{equation*}
...
\begin{align*}
    \frac{\delta S}{\delta \mc{A}^{K L} _{c}} & = \frac{T}{2} e e^{a} _{i} e^{b i} \frac{\partial}{\partial \mc{A}^{K L} _{c}} \left( \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} \right) \eta _{I J} = \\
                                              & = T e e^{a} _{i} e^{b i} k \delta ^{c} _{a} \delta ^{I} _{[K} \eta _{L] I'} X^{I'} \mc{D}_{b} X^{J} \eta _{I J} =                             \\
                                              & = T k e e^{c} _{i} e^{b i} \mc{D}_{b} X^{J} \eta _{J [K} X_{L]} \overset{!}{=} 0
\end{align*}
\begin{equation*}
    \mc{T}^{K L} _{c} = \mc{D}_{c} X^{[K} X^{L]} = 0
\end{equation*}
