...
\end{equation*}
\begin{equation*}
    \boxed{T^{i} _{a b} := e^{c i} D_{c} X^{\mu} _{k} v^{k} X^{\nu} _{[m} v_{n]} e^{m} _{a} e^{n} _{b} G_{\mu \nu} = 0}
\end{equation*}

\subsubsection{With extra field, w.r.t $v$}

\begin{align*}
    \frac{\delta S_{M P 3}}{\delta v^{l}} & = \frac{D \Lagr _{M P 3}}{D v^{l}} = \left( \frac{\partial}{\partial v^{l}} - \partial _{c} \frac{\partial}{\partial (\partial _{c} v^{l})} \right) \left( - \frac{T}{2} e^{a} _{i} e^{b i} D_{a} X^{\mu} _{j} v^{j} D_{b} X^{\nu} _{k} v^{k} E^{I} _{\mu} E_{\nu I} \right) = \\
