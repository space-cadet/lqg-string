...
\end{equation*}
where if $T' = - T k \Delta / 2$ and $h = g^{-1}$ we recover the NG area corrected action
\begin{equation*}
	S = - T \int \diff ^2 x \left( \sqrt{- g} - \frac{k \Delta}{2 \sqrt{- g}} \right) .
\end{equation*}
The condition $h = g^{-1}$ can be achieved by
\begin{equation*}
	h_{a b} = S_{a c} g^{c d} T_{d b} ,
\end{equation*}
where $S , T \in \mathrm{SL}(2 , \mathbb{C})$, so we have a gauge freedom in choosing $h_{a b}$. Since we require $[h_{a b}] = [h_{a b}]^{\dagger}$ ($h_{a b}$ must be symmetric and real), this means $[T_{d b}] = [S_{d b}]^{\dagger}$ and also that $[S_{a b}]^{\dagger} = [S_{a b}]$, thus
\begin{equation*}
	h_{a b} = S_{a c} g^{c d} S_{d b}.
\end{equation*}
This restriction constrains our gauge group to just the boosts part of $\mathrm{SL}(2 , \mathbb{C})$,
\begin{equation*}
	S = \e ^{- n^{i} \sigma _{i} \beta} .
\end{equation*}
This also implies
\begin{align*}
	\partial _{a} X^{\mu} \partial _{b} X^{\nu} H_{\mu \nu}                                       & = S_{a c} g^{c d} S_{d b} =                                                                                                                      \\
	                                                                                              & = S_{a c} \left( \frac{1}{2 g} \varepsilon ^{c c'} \varepsilon ^{d d'} g_{c' d'} \right) S_{d b} =                                               \\
	\delta _{a} ^{c'} \delta _{b} ^{d'} \partial _{c'} X^{\mu} \partial _{d'} X^{\nu} H_{\mu \nu} & = S_{a c} \left( \frac{1}{2 g} \varepsilon ^{c c'} \varepsilon ^{d d'} \partial _{c'} X^{\mu} \partial _{d'} X^{\nu} G_{\mu \nu} \right) S_{d b}
\end{align*}
\begin{equation*}
	\delta _{a} ^{c'} \delta _{b} ^{d'} H_{\mu \nu} = \frac{1}{2 g} S_{a c} \varepsilon ^{c c'} S_{d b} \varepsilon ^{d d'} G_{\mu \nu}
\end{equation*}
\begin{equation*}
	H_{\mu \nu} = - \frac{1}{8 g} S_{a c} \varepsilon ^{c a} S_{d b} \varepsilon ^{b d} G_{\mu \nu}
\end{equation*}
\begin{equation*}
	H_{\mu \nu} = \frac{1}{8 (- g)} \Tr (S \varepsilon) \Tr (S \varepsilon) G_{\mu \nu}
\end{equation*}
\begin{equation*}
	H_{\mu \nu} = \frac{1}{8 (- g)} \Tr (S \varepsilon)^2 G_{\mu \nu} .
\end{equation*}
This restricts $S$ to necessarily being not symmetric, otherwise the trace $\Tr (S \varepsilon)$ vanishes and we would get $H_{\mu \nu} = 0$.
...
\end{equation*}
Assuming the condition for $h = g^{-1}$ and flat background $G_{\mu \nu} = \eta _{\mu \nu}$, the EoM reduces to
\begin{equation*}
	\partial _{a} \left( \left( \sqrt{- g} g^{a b} - \frac{k \Delta}{2 (- g)} \sqrt{- g}^{-1} (S^{-1})^{a c} g_{c d} (S^{-1})^{d b} \right) \partial _{b} X^{\mu} \right) = 0 ,
\end{equation*}
which in conformal gauge $g_{a b} = \phi \eta _{a b}$ with $(S^{-1})^{a c} \eta _{c d} (S^{-1})^{d b} = \eta ^{a b}$ reduces further
\begin{equation*}
	\partial _{a} \left( \left( 1 - \frac{k \Delta}{2 \phi ^2} \right) \eta ^{a b} \partial _{b} X^{\mu} \right) = 0
\end{equation*}
\begin{equation*}
...
\end{equation*}
This is the exact same equation derived in the next section which leads to the Klein-Gordon eqn. The requirement that $(S^{-1})^{a c} \eta _{c d} (S^{-1})^{d b} = \eta ^{a b}$ implies
\begin{equation*}
	S = \begin{bmatrix} a & b \\ c & d \end{bmatrix}
\end{equation*}
\begin{equation*}
	S^{-1} = \begin{bmatrix} a & - b \\ - c & d \end{bmatrix} ,
\end{equation*}
\begin{equation*}
	S = \begin{bmatrix} \cosh \beta - n^3 \sinh \beta & - (n^{1} - i n^2) \sinh \beta \\ - (n^1 + i n^2) \sinh \beta & \cosh \beta + n^3 \sinh \beta \end{bmatrix} ,
\end{equation*}
\begin{equation*}
	S^{-1} = \begin{bmatrix} \cosh \beta + n^3 \sinh \beta & + (n^{1} - i n^2) \sinh \beta \\ + (n^1 + i n^2) \sinh \beta & \cosh \beta - n^3 \sinh \beta \end{bmatrix}
\end{equation*}
so $n^3 = 0$. Since $(n^1)^2 + (n^2)^2 = 1$, we can parametrize as $n^1 = \cos \theta$ and $n^2 = \sin \theta$, thus $S$ has the form
\begin{equation*}
	S = \begin{bmatrix} \cosh \beta & - \e ^{- i \theta} \sinh \beta \\ - \e ^{i \theta} \sinh \beta & \cosh \beta \end{bmatrix} ,
\end{equation*}
and $\Tr (S \varepsilon) = - 2 i \sin \theta \sinh \beta$. Let's try to find more restrictions on $S$:
\begin{align*}
	h^{a b} & = (S^{-1})^{a c} g_{c d} (S^{-1})^{d b}                                                                                                                                       \\
	h^{a b} & = \frac{1}{2 g^{-1}} \varepsilon ^{a a'} \varepsilon ^{b b'} \partial _{a'} X^{\mu} \partial _{b'} X^{\nu} \left( \frac{1}{8 (- g)} \Tr (S \varepsilon)^2 G_{\mu \nu} \right)
\end{align*}
\begin{equation*}
	\Downarrow
\end{equation*}
\begin{equation*}
	(S^{-1})^{a c} g_{c d} (S^{-1})^{d b} = \frac{1}{2 g^{-1}} \varepsilon ^{a a'} \varepsilon ^{b b'} \frac{1}{8 (- g)} \Tr (S \varepsilon)^2 \partial _{a'} X^{\mu} \partial _{b'} X^{\nu} G_{\mu \nu}
\end{equation*}
\begin{equation*}
	(S^{-1})^{a c} g_{c d} (S^{-1})^{d b} = - \frac{1}{16} \Tr (S \varepsilon)^2 \varepsilon ^{a c} \varepsilon ^{b d} g_{c d}
\end{equation*}
\begin{equation*}
	(S^{-1})^{a c} g_{c d} (S^{-1})^{d b} = \left( \frac{i}{4} \Tr (S \varepsilon) \varepsilon ^{a c} \right) g_{c d} \left( \frac{i}{4} \Tr (S \varepsilon) \varepsilon ^{d b} \right)
\end{equation*}
\begin{align*}
	g_{c d} = S_{c a} \left( \frac{i}{4} \Tr (S \varepsilon)^{*} \varepsilon ^{a c'} \right) g_{c' d'} \left( \frac{i}{4} \Tr (S \varepsilon) \varepsilon ^{d' b} \right) S_{b d}
\end{align*}
\begin{equation*}
	\Downarrow
\end{equation*}
\begin{align*}
	S_{c a} \left( \frac{i}{4} \Tr (S \varepsilon)^{*} \varepsilon ^{a c'} \right) & = \delta _{c} ^{c'} \\
	\left( \frac{i}{4} \Tr (S \varepsilon) \varepsilon ^{d' b} \right) S_{b d}     & = \delta _{d} ^{d'}
\end{align*}
\begin{equation*}
	\Downarrow
\end{equation*}
\begin{equation*}
	(S^{-1})^{a b} = \left( \frac{i}{4} \Tr (S \varepsilon) \varepsilon ^{a b} \right) ,
\end{equation*}
thus we have that
\begin{equation*}
	\begin{bmatrix} \cosh \beta & \e ^{- i \theta} \sinh \beta \\ e ^{i \theta} \sinh \beta & \cosh \beta \end{bmatrix} = \begin{bmatrix} 0 & \frac{1}{2} \sin \theta \sinh \beta \\ - \frac{1}{2} \sin \theta \sinh \beta & 0 \end{bmatrix}
\end{equation*}
\begin{equation*}
	\Downarrow
\end{equation*}
...
Turn embbeding fields $X^{\mu}$ into vector in the spin-1 representation of $\R ^{d + 1} \rtimes \mathrm{Spin}(d , 1) \simeq \mathrm{Spin} (d + 1 , 1)$, $X^{I}$, and promote partial derivative to covariant derivative $\partial _{a} \mapsto \mc{D}_{a}$ acting as
\begin{equation*}
	\mc{D}_{a} X^{I} = \partial _{a} X^{I} + k \mc{A}^{I} _{a J} X^{J} ,
\end{equation*}
where the WS $\R ^{d + 1} \rtimes \mathrm{Spin}(d , 1) \simeq \mathrm{Spin} (d + 1 , 1)$ connection $\mc{A}_{a}$ with $I,J = 0 , ... , d , d + 1$ is given by
\begin{equation*}
	\mc{A}_{a} X = \begin{bmatrix} 0                         & \beta ^{0 1} _{a}       & \beta ^{0 2} _{a}   & ... & \beta ^{0 d} _{a}       & p^{0} _{a} / l \\ \beta ^{0 1} _{a} & 0 & - \theta ^{1 2} _{a} & ... & - \theta ^{1 d} _{a} & p^{1} _{a} / l \\ \beta ^{0 2} _{a} & \theta ^{1 2} _{a} & 0 & ... & - \theta ^{2 d} _{a} & p^{2} _{a} / l \\ \vdots & \vdots & \vdots & \ddots & \vdots & \vdots \\ \beta ^{0 d} _{a} & \theta ^{1 d} _{a} & \theta ^{2 d} _{a} & ... & 0 & p^{d} _{a} / l \\
                - \epsilon p^{0} _{a} / l & \epsilon p^{1} _{a} / l & \epsilon p^{2} _{a} & ... & \epsilon p^{d} _{a} / l & 0\end{bmatrix} \begin{bmatrix} X^{0} \\ X^{1} \\ X^{2} \\ \vdots \\ X^{d} \\ 1 \end{bmatrix} = \left( \beta ^{0 \alpha} _{a} B_{0 \alpha} + \frac{1}{2} \theta ^{\alpha \beta} _{a} J_{\alpha \beta} + \frac{p^{\mu} _{a}}{l} P_{\mu} \right) X ,
\end{equation*}
...
\end{equation*}
Clearly, since $K$ is affected by gauge transformations and $\PS = \PS (X)$, $K$ is a bulk field $K = K (X)$.

\subsection{EoMs}

\subsubsection{W.r.t $K$}

\begin{align*}
	\frac{\delta S}{\delta K^{K L}} & \propto e e^{a} _{I'} e^{b} _{J'} \frac{\partial K^{I' J'}}{\partial K^{K L}} \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} K_{I J} + e e^{a} _{I'} e^{b} _{J'} K^{I' J'} \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} \frac{\partial K_{I J}}{\partial K^{K L}} = \\
	                                & = e e^{a} _{I'} e^{b} _{J'} \delta ^{I'} _{(K} \delta ^{J'} _{L)} \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} K_{I J} + e e^{a} _{I'} e^{b} _{J'} K^{I' J'} \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} \eta _{I (K} \eta _{L) J} =                             \\
	                                & = e e^{a} _{(K} e^{b} _{L)} \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} K_{I J} + e e^{a} _{I'} e^{b} _{J'} K^{I' J'} \mc{D}_{a} X_{(K} \mc{D}_{b} X_{L)} = 0
\end{align*}

\subsubsection{W.r.t $e$}

\begin{align*}
	\frac{\delta S}{\delta e^{c} _{K}} & \propto \left( \frac{\partial e}{\partial e^{c} _{K}} e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} + e \frac{\partial}{\partial e^{c} _{K}} (e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'}) \right) \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} \eta _{I J} = \\
	                                   & = \left( - e e^{K} _{c} e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} + 2 e \delta ^{a} _{c} \delta ^{K} _{I'} e^{b} _{J'} \eta ^{I' J'} \right) \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} \eta _{I J} =                                             \\
	                                   & = - e e^{K} _{c} e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} \eta _{I J} + 2 e e^{b} _{J'} \eta ^{K J'} \mc{D}_{c} X^{I} \mc{D}_{b} X^{J} \eta _{I J} = 0
\end{align*}
\begin{equation*}
	T^{K} _{c} := 2 e^{b} _{J'} \eta ^{K J'} \mc{D}_{c} X^{I} \mc{D}_{b} X^{J} \eta _{I J} - e^{K} _{c} e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} \eta _{I J} = 0
\end{equation*}
\begin{equation*}
	e^{K} _{c} = 2 f e^{b K} \mc{D}_{c} X^{I} \mc{D}_{b} X^{J} \eta _{I J} ,
\end{equation*}
\begin{equation*}
	\frac{1}{f} = e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} \eta _{I J}
\end{equation*}

\subsubsection{W.r.t $A$}

\begin{align*}
	\frac{\delta S}{\delta \mc{A}^{K L} _{c}} & = \frac{T}{2} \left( e e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} \frac{\partial}{\partial \mc{A}^{K L} _{c}} \left( \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} \right) \eta _{I J} \right) = \\
	                                          & = T \left( e e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} \delta ^{c} _{a} \delta ^{I} _{[K} \eta _{L] I'} X^{I'} \mc{D}_{b} X^{J} \eta _{I J} \right) =                               \\
	                                          & = T e e^{c} _{I'} e^{b} _{J'} \eta ^{I' J'} \mc{D}_{b} X_{[K} X_{L]} \overset{!}{=} 0
\end{align*}
\begin{equation*}
...
\begin{align*}
	\frac{\delta S}{\delta X^{K}} & = - \frac{T}{2} \left( \partial _{c} \left( e e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} \frac{\partial}{\partial (\partial _{c} X^{K})} \left( \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} \right) \eta _{I J} \right) - e e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} \frac{\partial}{\partial X^{K}} \left( \mc{D}_{a} X^{I} \mc{D}_{b} X^{J} \right) \eta _{I J} \right) = \\
	                              & = - T \left( \partial _{c} \left( e e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} \delta ^{c} _{a} \delta ^{I} _{K} \mc{D}_{b} X^{J} \eta _{I J} \right) - e e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} k \mc{A}^{I} _{a L} \delta ^{I'} _{L} \mc{D}_{b} X^{J} \eta _{I J} \right)=                                                                                    \\
	                              & = - T \left( \partial _{a} \left( e e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} \mc{D}_{b} X^{J} \eta _{K J} \right) - k \mc{A}^{I} _{a K} \left( e e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} \mc{D}_{b} X^{J} \eta _{I J} \right) \right) =                                                                                                                        \\
	                              & = - T \mc{D}_{a} \left( e e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} \mc{D}_{b} X_{K} \right) \overset{!}{=} 0
\end{align*}
\begin{equation*}
	\mc{D}_{a} \left( e e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} \mc{D}_{b} X^{I} \right) = 0
\end{equation*}
In conformal gauge $e e^{a} _{I'} e^{b} _{J'} \eta ^{I' J'} = \eta ^{a b}$, thus
\begin{equation*}
