...
\end{equation*}
where $T$ is the temperature of target-space, $\mu _{0}$ has units of mass (perhaps $\mu _{0} \sim M_{pl}$ or $\mu _{0} ^{2} \sim (\mathrm{quantum \ of \ area})^{-1}$?) and $m_{0} >> m_{1} >> ... >> m_{d}$. This action has manifest Lorentz invariance and WS reparameterisation and Weyl invariance. As for translational symmetry, it holds approximately for the fields with $m_{\mu} >> T$. The masses $m_{\mu}$ are constrained by the dispersion relations
\begin{equation*}
