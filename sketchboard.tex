...
\section{Quantum geometry Polyakov}
...
\end{align*}
where since $\alpha ^{\mu} _{- n} \propto \alpha ^{\mu} _{n}$ the relations for just positive $m , l$ are sufficient. We can rescale the vibrational modes to get harmonic oscillator bracket relations
\begin{align*}
...
\end{align*}
These relations reveal that, in fact, $\alpha ^{\mu} _{- n}$ = $\alpha ^{\mu} _{n}$, since the middle relation is only invariant to $p \mapsto - p$ if the proportionality coeficient is 1.
...
\begin{equation*}
    \bra{\mathrm{phys }'} \left( \sum _{n} \left( \left( 1 \mp \frac{n}{E_{n}} \right) ^2 \Hat{\widetilde{L}}_{n , n} \right) - c \right) \ket{\mathrm{phys}} = 0 ,
\end{equation*}
for some costant $c$. Since classically
\begin{align*}
    M^2 & = (2 c_{1})^2 \left( (c_{1} \mu)^{2} x^{\mu} x_{\mu} - \sum _{n \neq 0} \left( \left( 1 \mp \frac{n}{E_{n}} \right) ^2 \widetilde{L}_{n , n} \right) \right) =                                       \\
        & = (2 c_{1})^2 \left( (c_{1} \mu)^{2} x^{\mu} x_{\mu} - \sum _{n \neq 0} \left( \left( 1 \mp \frac{n}{E_{n}} \right) ^2 \alpha ^{\mu} _{n} (\alpha ^{\nu} _{n})^{*} \eta _{\mu \nu} \right) \right) ,
\end{align*}
we see that the string mass spectrum will be affected by this constant
\begin{equation*}
    \Hat{M}^2 = (2 c_{1})^{2} \left( (c_{1} \mu)^{2} \Hat{x}^{\mu} \Hat{x}_{\mu} - \left( \sum _{n \neq 0} \left( \left( 1 \mp \frac{n}{E_{n}} \right) ^2 E_{n} (\Hat{a}^{\mu} _{n})^{\dagger} \Hat{a}^{\nu} _{n} \eta _{\mu \nu} \right) - c \right) \right) .
\end{equation*}
...
\begin{equation*}
    \ket{\psi _{1 , n}} = \sum _{m} \Hat{\widetilde{L}}_{n , m} \ket{\widetilde{\chi}_{1 , m}}
\end{equation*}
where $\ket{\widetilde{\chi}_{1 , m}}$ satisfies
\begin{align*}
    \Hat{\widetilde{L}}_{k , p} \ket{\widetilde{\chi}_{1 , m}}                                                                                                                 & = 0 , \ k \neq - p \\
    \left( \sum _{n'} \left( \left( \left( \frac{n'}{E_{n'}} \right) ^2 - 1 \right) \Hat{\widetilde{L}}_{n' , - n'} \right) - c - 1 - m \right) \ket{\widetilde{\chi}_{1 , m}} & = 0 .
\end{align*}
These states are annihilated by $\Hat{\widetilde{L}}_{k , p}$, for $p \neq - n$, and for this case we have that
\begin{equation*}
    \sum _{n} \left( \left( \left( \frac{n}{E_{n}} \right) ^2 - 1 \right) \Hat{\widetilde{L}}_{2 , - n} \ket{\psi _{1 , n}} \right) =
\end{equation*}
\begin{equation*}
    = \sum _{n} \left( \left( \left( \frac{n}{E_{n}} \right) ^2 - 1 \right) \Hat{\widetilde{L}}_{2 , - n} \sum _{m} \Hat{\widetilde{L}}_{n , m} \ket{\widetilde{\chi}_{1 , m}} \right)  =
\end{equation*}
\begin{equation*}
    = \sum _{n} \left( \left( \left( \frac{n}{E_{n}} \right) ^2 - 1 \right) \Hat{\widetilde{L}}_{n , - n} \right) \ket{\widetilde{\chi}_{1 , - 2}} ,
\end{equation*}
which vanish if $c = 1$.

\newpage

\section{Area correction Path Integral}

Build the path integral of the area corrected Polyakov action
\begin{equation*}
    Z = \int D[g] D[X] \e ^{- i S} ,
\end{equation*}
\begin{equation*}
    S = - \frac{1}{2 \pi \alpha '} \int \diff ^2 x F^{-} \sqrt{- g} g^{a b} \del _{a} X^{\mu} \del _{b} X^{\nu} \eta _{\mu \nu} ,
\end{equation*}
\begin{equation*}
    F^{-} = 1 - \frac{k \Delta}{2 (- g)} .
\end{equation*}
In conformal gauge and choosing the conformal factor such that the EoM becomes a KG eqn,
\begin{equation*}
    F^{-} = \sum _{l} \left( A_{l} \e ^{i (E_{l} \tau + l \sigma)} + (A_{- l})^{*} \e ^{i (- E_{l} \tau + l \sigma)} \right) .
\end{equation*}
This will be the gauge chosen for performing the path integral.
