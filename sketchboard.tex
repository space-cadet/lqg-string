...
Starting from the mixed constraint, we have
\begin{equation*}
    \partial _{\tau} X^{\mu} \partial _{\sigma} X^{\nu} \eta _{\mu \nu} = - \frac{\eta _{\mu \nu}}{4} \sum _{n \in \mathbb{Z}} \sum _{k \in \mathbb{Z}} \frac{k}{E_{k}} \left( a^{\mu} _{n} \e ^{- i (- E_{n} \tau + n \sigma)} - b^{\mu} _{n} \e ^{i (- E_{n} \tau + n \sigma)} \right) \left( - a^{\nu} _{k} \e ^{- i (- E_{k} \tau + k \sigma)} + b^{\nu} _{k} \e ^{i (- E_{k} \tau + k \sigma)} \right) =
\end{equation*}
\begin{multline*}
    = - \frac{\eta _{\mu \nu}}{4} \sum _{n} \sum _{k} \frac{k}{E_{k}} \Big( - a^{\mu} _{n} a^{\nu} _{k} \e ^{i (E _{n} + E_{k}) \tau} \e ^{- i (n + k) \sigma} + a^{\mu} _{n} b^{\nu} _{k} \e ^{i (E_{n} - E_{k}) \tau} \e ^{- i (n - k) \sigma} + \\
    + b^{\mu} _{n} a^{\nu} _{k} \e ^{- i (E_{n} - E_{k}) \tau} \e ^{i (n - k) \sigma} - b^{\mu} _{n} b^{\nu} _{k} \e ^{- i (E_{n} + E_{k}) \tau} \e ^{i (n + k) \sigma} \Big) = 0 ,
\end{multline*}
for which we have 3 cases to analyse: $n = k$, $n = - k$ and $- k \neq n \neq k$. Starting with $- k \neq n \neq k$, since the equality must hold for all $\tau$ and $\sigma$ and the exponentials are linearly independent, we get that each term must vanish:
\begin{align*}
    a^{\mu} _{n} a^{\nu} _{k} \eta _{\mu \nu} & = 0                         \\
    a^{\mu} _{n} b^{\nu} _{k} \eta _{\mu \nu} & = 0                         \\
    b^{\mu} _{n} a^{\nu} _{k} \eta _{\mu \nu} & = 0                         \\
    b^{\mu} _{n} b^{\nu} _{k} \eta _{\mu \nu} & = 0 , \ - k \neq n \neq k .
\end{align*}
For $n = k$, the 2 middle terms become linearly dependent, thus we get
\begin{align*}
    a^{\mu} _{n} a^{\nu} _{n} \eta _{\mu \nu}                               & = 0   \\
    (a^{\mu} _{n} b^{\mu} _{n} + b^{\mu} _{n} a^{\nu} _{n}) \eta _{\mu \nu} & = 0   \\
    b^{\mu} _{n} b^{\nu} _{n} \eta _{\mu \nu}                               & = 0 ,
\end{align*}
and finally for $n = - k$, this time the $\tau$ exponential still survives since $E_{n}$ is symmetric, thus we get
\begin{align*}
    a^{\mu} _{n} a^{\nu} _{- n} \eta _{\mu \nu} & = 0   \\
    a^{\mu} _{n} b^{\nu} _{- n} \eta _{\mu \nu} & = 0   \\
    b^{\mu} _{n} a^{\nu} _{- n} \eta _{\mu \nu} & = 0   \\
    b^{\mu} _{n} b^{\nu} _{- n} \eta _{\mu \nu} & = 0 .
\end{align*}
The first and last conditions in each group can be combined into
\begin{align*}
    a^{\mu} _{n} a^{\nu} _{k} \eta _{\mu \nu} & = 0                                      \\
    b^{\mu} _{n} b^{\nu} _{k} \eta _{\mu \nu} & = 0 , \ \forall \ n , k \in \mathbb{Z} .
\end{align*}
The middle conditions can be summed to combine and become
\begin{equation*}
    (a^{\mu} _{n} b^{\nu} _{k} + b^{\nu} _{n} a^{\mu} _{k}) \eta _{\mu \nu} = 0 , \ \forall \ n , k \in \mathbb{Z} .
\end{equation*}
Repeating the analysis for the other constraint will give the same results.

So far, we have thus that $a$ and $b$ for different values of $n$ are orthogonal to each other, and in particular when the index are equal. Thus, they are null vectors, and remain always colinear since that's the only way two null vectors can have a 0 dot product. Altogether, this leads to
\begin{equation*}
    a^{\mu} _{n} = a_{n} a^{\mu} _{0} , \ a_{0} = 1 , \ a^{\mu} _{0} a^{\nu} _{0} \eta _{\mu \nu} = 0
\end{equation*}
\begin{equation*}
    b^{\mu} _{n} = b_{n} a^{\mu} _{n} = b_{n} a_{n} a^{\mu} _{0} .
\end{equation*}
Without loss of generality, we can choose background coordinates such that $a^{\mu} _{0} = (\frac{1}{\sqrt{2}} , \frac{1}{\sqrt{2}} , 0 , ... , 0) = x^{+}$, giving us
\begin{equation*}
    X^{\mu} (\tau , \sigma) = a^{\mu} _{0} \sum _{n} \frac{a_{n}}{2 E_{n}} \left( \e ^{- i (- E_{n} \tau + n \sigma)} + b_{n} \e ^{i (- E_{n} \tau + n \sigma)} \right)
\end{equation*}
