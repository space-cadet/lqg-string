...
\end{equation*}
One way this can hold is if the expression in parenthesis is equal to 0, which can be achieved if we choose $S = i \varepsilon$, leaving us with
\begin{equation*}
    \delta _{a} ^{c'} \delta _{b} ^{d'} H_{\mu \nu} - \frac{1}{2 g} \delta _{a} ^{c'} \delta _{b} ^{d'} G_{\mu \nu} = 0
\end{equation*}
\begin{equation*}
    H_{\mu \nu} = \frac{1}{2 g} G_{\mu \nu} .
\end{equation*}
...
\begin{equation*}
    \partial _{a} \left( \left( \sqrt{- g} g^{a b} - \frac{k \Delta}{4 g} \sqrt{- g}^{-1} (i \varepsilon ^{a c}) g_{c d} (i \varepsilon ^{d b}) \right) \partial _{b} X^{\mu} \right) = 0
\end{equation*}
\begin{equation*}
...
\end{equation*}
which in conformal gauge $g_{a b} = \phi \eta _{a b}$ reduces further
\begin{equation*}
...
\end{equation*}
This is the exact same equation derived in the next section which leads to the Klein-Gordon eqn. In fact, plugging the condition for $h = g^{-1}$ and $T' = - T k \Delta / 2$ in the action reproduces the action in next section:
\begin{equation*}
    S = - \frac{T}{2} \int \diff ^2 x \sqrt{- g} g^{a b} \del _{a} X^{\mu} \del _{b} X^{\nu} G_{\mu \nu} + \frac{T k \Delta}{4} \int \diff ^2 x \sqrt{- g}^{-1} (i \varepsilon ^{a c}) g_{c d} (i \varepsilon ^{d b}) \del _{a} X^{\mu} \del _{b} X^{\nu} \frac{G_{\mu \nu}}{2 g} =
\end{equation*}
\begin{equation*}
...
\end{equation*}
\begin{equation*}
    \frac{i k \Delta \e ^{- 2 \phi}}{1 - \dfrac{k \Delta \e ^{- 2 \phi}}{2}} (E \partial _{\tau} \phi - p \partial _{\sigma} \phi) = - m^2
\end{equation*}
\begin{equation*}
    \frac{2 i}{\dfrac{2 \e ^{2 \phi}}{k \Delta} - 1} (E \partial _{\tau} \phi - p \partial _{\sigma} \phi) = - m^2
\end{equation*}
\begin{equation*}
    m^2 = \frac{2 i}{\dfrac{2 \e ^{2 \phi}}{k \Delta} - 1} (p \partial _{\sigma} \phi - E \partial _{\tau} \phi) .
\end{equation*}
...
\end{multline*}
\begin{equation*}
    M^2 \zeta ^{\mu} \zeta _{\mu} = \frac{8}{\alpha '} \left( \frac{2 \mu ^2}{\alpha '} \left| \frac{\partial \psi (p)}{\partial p} \right| ^2 \zeta ^{\mu} \zeta _{\mu} - \left( 2 \left( 1 - \frac{1}{\sqrt{1 + \mu ^2}} \right) ^2 \sqrt{1 + \mu ^2} \right) \zeta ^{\mu} \zeta _{\mu} + c \zeta ^{\mu} \zeta _{\mu} \right)
\end{equation*}
\begin{equation*}
    \frac{\alpha '}{8} M^2 = - \left( \frac{2 \mu ^2}{\alpha '} \left| \frac{\partial \psi (p)}{\partial p} \right| ^2 - \left( 2 \left( (1 + \mu ^2)^{1/4} - (1 + \mu ^2)^{- 1/4} \right) ^2 \right) + c \right)
\end{equation*}
\begin{equation*}
    \frac{\alpha '}{8} M^2 = - \left( \frac{2 \mu ^2}{\alpha '} \left| \frac{\partial \psi (p)}{\partial p} \right| ^2 - 4 \sinh ^2 \left( \frac{1}{4} \ln (1 + \mu ^2) \right) + c \right)
\end{equation*}
...
\end{equation*}
\begin{equation*}
    \zeta _{\mu} \left( \sqrt{\frac{2}{\alpha '}} \mu \Hat{x}^{\mu} - \frac{1}{2} \sqrt{\frac{\alpha '}{2}} i \Hat{p}^{\mu} \right) \ket{0 ; p} = 0
\end{equation*}
\begin{equation*}
...
\begin{equation*}
    \frac{\alpha '}{8} p^2 = - \left( \frac{2 \mu ^2}{\alpha '} \frac{(\alpha ')^2}{16 \mu ^2} p^2 - 4 \sinh ^2 \left( \frac{1}{4} \ln (1 + \mu ^2) \right) + c \right)
\end{equation*}
\begin{equation*}
    \frac{\alpha '}{4} p^2 = 4 \sinh ^2 \left( \frac{1}{4} \ln (1 + \mu ^2) \right) - c ,
\end{equation*}
and for the ground state
\begin{equation*}
    \frac{\alpha '}{4} p^2 = 4 \sinh ^2 \left( \frac{1}{4} \ln (\mu ^2) \right) - c .
\end{equation*}
In general, the $n$-th excited state will have momentum given by
\begin{equation*}
    \frac{\alpha '}{4} p^2 = 4 \sinh ^2 \left( \frac{1}{4} \ln (n^2 + \mu ^2) \right) - c .
\end{equation*}
...
\begin{equation*}
    c \geq 4 \sinh ^2 \left( \frac{1}{4} \ln (d^2 + \mu ^2) \right) .
\end{equation*}
In the boundary case, the $d$-th excited vector state is massless and the rest (including the scalar ground state) are regular massive states (no tachyons!). This analysis, however, is to get spin 1 particles from the closed string. The actual massless particle we want from closed strings is a symmetric spin 2 particle, so let's consider now the state $\zeta _{\mu \nu} (\Hat{a}^{\mu} _{d})^{\dagger} (\Hat{a}^{\nu} _{d})^{\dagger} \ket{0 ; p}$ where $\zeta _{\mu \nu} = \zeta _{\nu \mu}$ with $\eta ^{\mu \nu} \zeta _{\mu \nu}$ is the polarization tensor. The mass-shell is
\begin{multline*}
    ...
    - \Bigg( \sum _{n \neq 0} \left( \left( 1 \mp \frac{n}{E_{n}} \right) ^2 E_{n} \right) \bra{0 ; p} \Hat{a}^{\alpha} _{d} \Hat{a}^{\beta} _{d} \zeta _{\alpha \beta} (\Hat{a}^{\mu '} _{n})^{\dagger} \Hat{a}^{\nu '} _{n} \eta _{\mu ' \nu '} \zeta _{\mu \nu} (\Hat{a}^{\mu} _{d})^{\dagger} (\Hat{a}^{\nu} _{d})^{\dagger} \ket{0 ; p} - \bra{0 ; p} \Hat{a}^{\alpha} _{d} \Hat{a}^{\beta} _{d} \zeta _{\alpha \beta} c \zeta _{\mu \nu} (\Hat{a}^{\mu} _{d})^{\dagger} (\Hat{a}^{\nu} _{d})^{\dagger} \ket{0 ; p} \Bigg) \Bigg)
\end{multline*}
\begin{equation*}
    M^2 \zeta _{\mu \nu} \zeta ^{\mu \nu} = \frac{8}{\alpha '} \Bigg( \frac{2 \mu ^2}{\alpha '} \left| \frac{\partial \psi (p)}{\partial p} \right| ^2 \zeta _{\mu \nu} \zeta ^{\mu \nu} - \left( 4 \left( \left( 1 - \frac{d}{\sqrt{d^2 + \mu ^2}} \right) ^2 \sqrt{d^2 + \mu ^2} \right) \zeta _{\mu \nu} \zeta ^{\mu \nu} - c \zeta _{\mu \nu} \zeta ^{\mu \nu} \right) \Bigg)
\end{equation*}
\begin{equation*}
    \frac{8}{\alpha '} p^2 = - \Bigg( \frac{2 \mu ^2}{\alpha '} \left| \frac{\partial \psi (p)}{\partial p} \right| ^2 - \left( 4 \left( (d^2 + \mu ^2)^{1/4} - (d^2 + \mu ^2)^{- 1/4} \right) - c \right) \Bigg)
\end{equation*}
\begin{equation*}
    \frac{\alpha '}{4} p^2 = 4 \left( (d^2 + \mu ^2)^{1/4} - (d^2 + \mu ^2)^{- 1/4} \right) - c
\end{equation*}
\begin{equation*}
    \frac{\alpha '}{4} p^2 = 8 \sinh ^2 \left( \frac{1}{4} \ln (d^2 + \mu ^2) \right) - c .
\end{equation*}
...
\begin{equation*}
    \zeta _{\mu \nu} p^{\mu} = \zeta _{\mu \nu} p^{\nu} = 0 .
\end{equation*}
Since the norm of the spin-2 states is given by $\zeta ^{\mu \nu} \zeta _{\mu \nu}$, we need those to not be negative. Since $\zeta$ is symmetric and we work in $(- , + , + , ...)$ signature, $\zeta ^{\mu \nu} \zeta _{\mu \nu} \geq 0$ always hold, so we just need to choose $c$ such that we get at leats 1 massless spin-2 state. For that, we see that
\begin{equation*}
    c = 8 \sinh ^2 \left( \frac{1}{4} \ln (d^2 + \mu ^2) \right)
\end{equation*}
does the trick for the $d$-th state, and since for the non-existence of ghosts in the spin-1 states we required
\begin{equation*}
    c \geq 4 \sinh ^2 \left( \frac{1}{4} \ln (d^2 + \mu ^2) \right) ,
\end{equation*}
which is smaller than the condition for the spin-2, so we can safely choose
\begin{equation*}
    c \equiv 8 \sinh ^2 \left( \frac{1}{4} \ln (d^2 + \mu ^2) \right) ,
\end{equation*}
leaving the $d$-th spin-2 state massless, the rest spin-2 and all spin-1 particles being massive.
