...
\end{align*}
Note that $(\Hat{\widetilde{L}}_{n , p})^{\dagger} = \frac{(p - n)}{E_{p - n}} \frac{E_{n}}{(- n)} \Hat{\widetilde{L}}_{n - p , - p}$ thus if we have a sequence of $L$'s sandwiched between physical states (where if one of the $p$'s is 0 the $\widetilde{L}$ should be replaced by $\widetilde{L} - a$)
\begin{equation*}
