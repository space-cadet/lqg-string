...
\end{equation*}
with $X^{A} = (X^{\mu} , X^{r})$ and action thus becomes
\begin{equation*}
    S = - \frac{T}{2} \int \diff ^2 x \sqrt{- g} g^{a b} \mc{D}_{a} X^{A} \mc{D}_{b} X^{B} G_{A B} ,
\end{equation*}
where $G_{A B}$ is the expanded metric, which we'll assume to be
\begin{equation*}
    [G_{A B}] = \begin{bmatrix} [\eta _{\mu \nu}] & [G_{\mu r}] \\ [G_{\mu r}]^{T} & G_{r r} \end{bmatrix} ,
\end{equation*}
thus separating the action into $S = S [X^{\mu}] + S_{int} [X^{\mu} , X^{r}] + S [X^{r}]$
\begin{align*}
    S [X^{\mu}] & = - \frac{T}{2} \int \diff ^2 x \sqrt{- g} g^{a b} \mc{D}_{a} X^{\mu} \mc{D}_{b} X^{\nu} \eta _{\mu \nu} \\
    S_{int} [X^{\mu} , X^{r}] & = - T \int \diff ^2 x \sqrt{- g} g^{a b} \mc{D}_{a} X^{\mu} \mc{D}_{b} X^{r} G_{\mu r} \\
    S [X^{r}] & = - \frac{T}{2} \int \diff ^2 x \sqrt{- g} g^{a b} \mc{D}_{a} X^{r} \mc{D}_{b} X^{r} G_{r r} .
\end{align*}
...
\end{equation*}
In conformal gauge $g^{a b} = \e ^{- \phi} \eta ^{a b}$ and assuming $G_{A B} = \eta ' _{A B}$, where
\begin{equation*}
