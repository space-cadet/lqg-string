...
\end{equation*}
where $S , T \in \mathrm{SL}(2 , \mathbb{C})$, so we have a gauge freedom in choosing $h_{a b}$. Since we require $[h_{a b}] = [h_{a b}]^{\dagger}$ ($h_{a b}$ must be symmetric and real), this means $[T_{d b}] = [S_{d b}]^{\dagger}$, thus
\begin{equation*}
    h_{a b} = S_{a c} g^{c d} S^{\dagger} _{d b}.
\end{equation*}
This also implies
\begin{align*}
    \partial _{a} X^{\mu} \partial _{b} X^{\nu} H_{\mu \nu}                                       & = S_{a c} g^{c d} S^{\dagger} _{d b} =                                                                                                                      \\
                                                                                                  & = S_{a c} \left( \frac{1}{2 g} \varepsilon ^{c c'} \varepsilon ^{d d'} g_{c' d'} \right) S^{\dagger} _{d b} =                                               \\
    \delta _{a} ^{c'} \delta _{b} ^{d'} \partial _{c'} X^{\mu} \partial _{d'} X^{\nu} H_{\mu \nu} & = S_{a c} \left( \frac{1}{2 g} \varepsilon ^{c c'} \varepsilon ^{d d'} \partial _{c'} X^{\mu} \partial _{d'} X^{\nu} G_{\mu \nu} \right) S^{\dagger} _{d b}
\end{align*}
\begin{equation*}
    \delta _{a} ^{c'} \delta _{b} ^{d'} H_{\mu \nu} = \frac{1}{2 g} S_{a c} \varepsilon ^{c c'} S^{\dagger} _{d b} \varepsilon ^{d d'} G_{\mu \nu}
\end{equation*}
\begin{equation*}
    H_{\mu \nu} = - \frac{1}{8 g} S_{a c} \varepsilon ^{c a} S^{\dagger} _{d b} \varepsilon ^{b d} G_{\mu \nu}
\end{equation*}
\begin{equation*}
    H_{\mu \nu} = \frac{1}{8 (- g)} \Tr (S \varepsilon) \Tr (S^{\dagger} \varepsilon) G_{\mu \nu}
\end{equation*}
\begin{equation*}
    H_{\mu \nu} = \frac{1}{8 (- g)} \Tr (S \varepsilon) \Tr (S^{\dagger} (- \varepsilon ^{\dagger})) G_{\mu \nu}
\end{equation*}
\begin{equation*}
    H_{\mu \nu} = \frac{1}{8 g} \Tr (S \varepsilon) \Tr ((\varepsilon S)^{\dagger}) G_{\mu \nu}
\end{equation*}
\begin{equation*}
    H_{\mu \nu} = \frac{1}{8 g} \Tr (S \varepsilon) \Tr (\varepsilon S)^{*} G_{\mu \nu}
\end{equation*}
\begin{equation*}
...
\begin{equation*}
    \partial _{a} \left( \left( \sqrt{- g} g^{a b} - \frac{k \Delta}{2 (- g)} \sqrt{- g}^{-1} ((S^{\dagger})^{-1})^{a c} g_{c d} (S^{-1})^{d b} \right) \partial _{b} X^{\mu} \right) = 0 ,
\end{equation*}
which in conformal gauge $g_{a b} = \phi \eta _{a b}$ with $((S^{\dagger})^{-1})^{a c} \eta _{c d} (S^{-1})^{d b} = \eta ^{a b}$ reduces further
\begin{equation*}
...
\end{equation*}
This is the exact same equation derived in the next section which leads to the Klein-Gordon eqn. The requirement that $((S^{\dagger})^{-1})^{a c} \eta _{c d} (S^{-1})^{d b} = \eta ^{a b}$ restricts our gauge group to $\mathrm{SU}(1 , 1)$. Let's try to find more restrictions on $S$:
\begin{align*}
    h^{a b} & = ((S^{\dagger})^{-1})^{a c} g_{c d} (S^{-1})^{d b}                                                                                                                         \\
...
\end{equation*}
\begin{equation*}
    ((S^{\dagger})^{-1})^{a c} g_{c d} (S^{-1})^{d b} = \frac{1}{2 g^{-1}} \varepsilon ^{a a'} \varepsilon ^{b b'} \frac{1}{8 g} |\Tr (S \varepsilon)|^2 \partial _{a'} X^{\mu} \partial _{b'} X^{\nu} G_{\mu \nu}
\end{equation*}
\begin{equation*}
    ((S^{\dagger})^{-1})^{a c} g_{c d} (S^{-1})^{d b} = \frac{1}{16} |\Tr (S \varepsilon)|^2 \varepsilon ^{a c} \varepsilon ^{b d} g_{c d}
\end{equation*}
\begin{equation*}
    ((S^{\dagger})^{-1})^{a c} g_{c d} (S^{-1})^{d b} = \left( \frac{i}{4} \Tr (S \varepsilon)^{*} \varepsilon ^{a c} \right) g_{c d} \left( \frac{i}{4} \Tr (S \varepsilon) \varepsilon ^{d b} \right)
\end{equation*}
\begin{align*}
    g_{c d} = S^{\dagger} _{c a} \left( \frac{i}{4} \Tr (S \varepsilon)^{*} \varepsilon ^{a c'} \right) g_{c' d'} \left( \frac{i}{4} \Tr (S \varepsilon) \varepsilon ^{d' b} \right) S_{b d}
\end{align*}
\begin{equation*}
    \Downarrow
\end{equation*}
\begin{align*}
    S^{\dagger} _{c a} \left( \frac{i}{4} \Tr (S \varepsilon)^{*} \varepsilon ^{a c'} \right) & = \delta _{c} ^{c'} \\
...
\begin{align*}
    ...
    ((S^{-1})^{\dagger})^{a b} & = \left( \frac{i}{4} \Tr (S \varepsilon)^{*} \varepsilon ^{a b} \right) ,
\end{align*}
however, since $S \in \mathrm{SU}(1 , 1)$, $S^{-1} = \eta S^{\dagger} \eta$, thus
\begin{equation*}
    \left( \frac{i}{4} \Tr (S \varepsilon) \varepsilon ^{a b} \right) = \eta ^{a a'} S^{\dagger} _{a' b'} \eta ^{b' b} .
\end{equation*}
Taking the determinant, we get
\begin{equation*}
    - \frac{1}{16} (\Tr (S \varepsilon))^2 = 1
\end{equation*}
\begin{equation*}
    (\Tr (S \varepsilon))^2 = - 16
\end{equation*}
\begin{equation*}
    \Tr (S \varepsilon) = \pm 4 i .
\end{equation*}
Without loss of generality, we can choose the negative branch $\Tr (S \varepsilon) = - 4 i$, thus leaving us with
\begin{equation*}
    \varepsilon ^{a b} = \eta ^{a a'} S^{\dagger} _{a' b'} \eta ^{b' b}
\end{equation*}
\begin{equation*}
    - \varepsilon _{c d} = S^{\dagger} _{c d}
\end{equation*}
\begin{equation*}
    \Downarrow
\end{equation*}
\begin{equation*}
    S_{a b} = \varepsilon _{a b} ...
\end{equation*}
this can't be right, since $\Tr (\varepsilon ^2) = - 2$, not $- 4 i$...
