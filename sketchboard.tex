...
\begin{align*}
    ...
    (\Hat{a}^{\mu} _{n})^{\dagger} & := (\Hat{\alpha} ^{\mu} _{- n})^{\dagger} , \ n > 0 .
\end{align*}

We define a vacuum state of the string to obey
\begin{equation*}
    \Hat{a}^{\mu} _{n} \ket{0} = 0 , \ \mathrm{for} \ n > 0 .
\end{equation*}
For $n = 0$, we have the center of mass momentum operator, so the vacuum also obeys
\begin{equation*}
