...
\begin{align*}
    \frac{\delta S_{c}}{\delta g^{c d}} & = \left( \frac{\partial \sqrt{- g}^{-1}}{\partial g^{c d}} \left( \sqrt{k \Delta} g^{a b} - \delta ^{a b} \right) - \sqrt{k \Delta} \sqrt{- g}^{-1} \frac{\partial g^{a b}}{\partial g^{c d}} \right) \partial _{a} X^{\mu} \partial _{b} X^{\nu} G_{\mu \nu} = \\
                                        & = \left( - \frac{1}{2 \sqrt{- g}^{-1}} g_{c d} \left( \sqrt{k \Delta} g^{a b} - \delta ^{a b} \right) - \sqrt{k \Delta} \sqrt{- g}^{-1} \delta ^{(a} _{c} \delta ^{b)} _{d} \right) \partial _{a} X^{\mu} \partial _{b} X^{\nu} G_{\mu \nu} =                   \\
                                        & = - \left( \frac{1}{2} \sqrt{- g} g_{c d} \left( \sqrt{k \Delta} g^{a b} - \delta ^{a b} \right) + \sqrt{k \Delta} \sqrt{- g}^{-1} \delta ^{a} _{(c} \delta ^{b} _{d)} \right) \partial _{a} X^{\mu} \partial _{b} X^{\nu} G_{\mu \nu}
\end{align*}
\begin{equation*}
    \Downarrow
\end{equation*}
\begin{multline*}
    T_{c d} := \sqrt{- g} \partial _{c} X^{\mu} \partial _{d} X^{\nu} G_{\mu \nu} - \frac{1}{2} \sqrt{- g} g_{c d} g^{a b} \partial _{a} X^{\mu} \partial _{b} X^{\nu} G_{\mu \nu} - \\
    - \frac{\sqrt{k \Delta}}{2} \left( \sqrt{k \Delta} \sqrt{- g}^{-1} \partial _{c} X^{\mu} \partial _{d} X^{\nu} G_{\mu \nu} + \frac{\sqrt{- g}}{2} g_{c d} \left( \sqrt{k \Delta} g^{a b} - \delta ^{a b} \right) \partial _{a} X^{\mu} \partial _{b} X^{\nu} G_{\mu \nu} \right) = 0
\end{multline*}
\begin{equation*}
    T_{c d} = \left( 1 - \frac{k \Delta}{2 (- g)} \right) \partial _{c} X^{\mu} \partial _{d} X^{\nu} G_{\mu \nu} - \frac{1}{2} g_{c d} \left( \left( 1 + \frac{k \Delta}{2} \right) g^{a b} - \frac{\sqrt{k \Delta}}{2} \delta ^{a b} \right) \partial _{a} X^{\mu} \partial _{b} X^{\nu} G_{\mu \nu} = 0
\end{equation*}
\begin{equation*}
    \Downarrow
\end{equation*}
\begin{equation*}
    g_{c d} = 2 \frac{F^{-}}{f'} \partial _{c} X^{\mu} \partial _{d} X^{\nu} G_{\mu \nu} , \ f' = \left( \left( 1 + \frac{k \Delta}{2} \right) g^{a b} - \frac{\sqrt{k \Delta}}{2} \delta ^{a b} \right) \partial _{a} X^{\mu} \partial _{b} X^{\nu} G_{\mu \nu}
\end{equation*}
...
\end{equation*}
This, however, does not amount to any motion in actual space-time, since with trivial connection the vector does not change after parallel transport. For a more interesting case, let's consider $A_{\tau} ^{\alpha \beta} = A_{\sigma} ^{\alpha \beta} = a \varepsilon ^{\alpha \beta}$, which amounts to flat background. The EoM turn into
\begin{equation*}
    \left( \partial _{\sigma} ^2 - \partial _{\tau} ^2 \right) X^{I} + 2 k a (\varepsilon ^{\alpha \beta} T_{\alpha \beta})^{I} _{\ J} \left( \partial _{\sigma} - \partial _{\tau} \right) X^{J} + k^2 a^2 \left( \left( \varepsilon ^{\alpha \beta} T_{\alpha \beta} \right) ^{I} _{\ I'} \left( \varepsilon ^{\alpha ' \beta '} T_{\alpha ' \beta '} \right) ^{I'} _{\ J} \right) X^{J} = 0
\end{equation*}

\newpage

\begin{equation*}
    \varepsilon ^{\alpha \beta} T_{\alpha \beta} = 2 \sum _{\alpha , \beta} T_{\alpha \beta} = 2 \begin{bmatrix} 0 & 1 & 1 & ... & 1 \\ 1 & 0 & 1 & ... & (-1)^{d} \\ 1 & - 1 & 0 & ... & (-1)^{d - 1} \\ \vdots & \vdots & \vdots & \ddots & \vdots \\ 1 & (-1)^{d + 1} & (-1)^{d} & ... & 0 \end{bmatrix}
\end{equation*}
