...
Turn embbeding fields $X^{\mu}$ into field with internal index $X^{I}$, and promote partial derivative to covariant derivative $\partial _{a} \mapsto D_{a}$ acting as
\begin{equation*}
    D_{a} X^{I} = \partial _{a} X^{I} + k A^{I} _{a J} X^{J} ,
\end{equation*}
where $A^{I J} _{a}$ is the background $\mathrm{Spin}(d , 1)$ connection, so the action becomes
\begin{equation*}
    S = - \frac{T}{2} \int \diff ^2 x \sqrt{g} g^{a b} D_{a} X^{I} D_{b} X^{J} \eta _{I J} ,
\end{equation*}
where the metrics can be recast in terms of vielbein fields as
\begin{equation*}
    S = - \frac{T}{2} \int \diff ^2 x e e^{a} _{K} e^{b K} D_{a} X^{I} D_{b} X^{J} E^{\mu} _{I} E^{\nu} _{J} G_{\mu \nu}
\end{equation*}
