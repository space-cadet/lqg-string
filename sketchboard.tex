...
Analysing the constraints, we have that
\begin{equation*}
...
\end{multline*}
Since the exponentials are all linearly independent and we can't decouple the tau exponential from any of the sums, we have that all 4 coefficients must vanish independently:
\begin{align*}
...
\end{align*}
$L^{2} _{n , p}$ and $L^{3} _{n , p}$ are just complex conjugates of each other with $n \mapsto - n$ and $p \mapsto - p$, so they are redundant. The same is true for $L^{1} _{n , p}$ and $L^{4} _{n , p}$, so in the end we have
\begin{align*}
