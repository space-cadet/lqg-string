...
\end{equation*}
This leaves 5 polarizations in $D = 4$. This is the expected 6 from massive gravitons minus one from the traceless condition.
...
\section{Path Integral for the area corrected Polyakov action}

Let's try to construct the transition amplitude for the area corrected string
\begin{equation*}
	W [g , X] = \mc{N} \int [\mc{D} g] [\mc{D} X] \e ^{i \frac{T}{2} \int \diff ^2 x \left( \sqrt{- g} - \frac{k \Delta}{2 \sqrt{- g}} \right) g^{a b} \partial _{a} X^{\mu} \partial _{b} X^{\nu} \eta _{\mu \nu}}
\end{equation*}
...
\end{equation*}
In conformal gauge $e e^{a} _{I'} e^{b} _{J'} K^{I' J'} = \eta ^{a b}$ and in \textquotedblleft constant gauge" $K (X) = K \ constant$, this reduces to
\begin{equation*}
	\eta ^{a b} \mc{D}_{a} \mc{D}_{b} X^{I} K_{I K} = 0 .
\end{equation*}
More explicitly,
\begin{equation*}
