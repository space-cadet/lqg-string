...
The action\eqref{eqn:modified-NG-action} is still manifestly Poincaré invariant, since it only contains derivatives of $X^{\mu}$, thus it's variations vanish under global translations as in
\begin{equation} \label{eqn:global-inf-transl-NG}
	X^{\mu} \mapsto X^{\mu} + \epsilon ^{\mu} ,
\end{equation}
thus $\delta (\partial _{a} X^{\mu}) = \partial _{a} (\delta X^{\mu}) = \partial _{a} \epsilon ^{\mu} \equiv 0$. As for Lorentz transformations, we have that
\begin{equation} \label{eqn:global-inf-LT-NG}
	X^{\mu} \mapsto X^{\mu} + \epsilon ^{\mu \nu} X_{\nu} ,
\end{equation}
and since every term in the action is of the form
\begin{equation} \label{eqn:NG-general-term-form}
	\partial _{a} X^{\mu} \partial _{b} X^{\nu} \eta _{\mu \nu} ,
\end{equation}
with $\epsilon ^{\mu \nu} = - \epsilon ^{\nu \mu}$, we see immediately that the transformation leaves the action unchanged:
\begin{equation*}
	\delta (\partial _{a} X^{\mu} \partial _{b} X^{\nu} \eta _{\mu \nu}) =
\end{equation*}
\begin{equation*}
	= (\partial _{a} (\delta X^{\mu}) \partial _{b} X^{\nu} + \partial _{a} X^{\mu} \partial _{b} (\delta X^{\nu})) \eta _{\mu \nu} =
\end{equation*}
\begin{equation*}
	= (\epsilon ^{\mu \rho} \partial _{a} X_{\rho} \partial _{b} X^{\nu} + \partial _{a} X^{\mu} \partial _{b} X_{\rho} \epsilon ^{\nu \rho}) \eta _{\mu \nu} =
\end{equation*}
\begin{equation*}
	= \epsilon _{\nu \mu} \partial _{a} X^{\mu} \partial _{b} X^{\nu} + \epsilon _{\mu \nu} \partial _{a} X^{\mu} \partial _{b} X^{\nu} =
\end{equation*}
\begin{equation} \label{eqn:vanishing-inf-LT-NG}
	= (\epsilon _{\mu \nu} + \epsilon _{\nu \mu}) \partial _{a} X^{\mu} \partial _{b} X^{\nu} \equiv 0
\end{equation}
by the anti-symmetry of $\epsilon _{\mu \nu}$. At face value, this action does not appear to satisfy diffeomorphism invariance of the string worldsheet because of the second term. This stems from the fact that we are assuming the existence of a minimum quantum of area, so it is obvious that diffeomorphism invariance, at least in the classical sense, will no longer be an exact symmetry of the theory.

In the expression\eqref{eqn:modified-NG-action} we can see a duality between the first and second terms. If we replace the metric $h_{ab}$ by its inverse $h'_{ab} \equiv h^{ab}$, then the action in terms of $h'_{ab}$ is the same as \eqref{eqn:modified-NG-action}, but with the difference that the factor of $g \Delta/2$ is mapped to $2/(g\Delta)$:
...
In the following we parameterise the string with $\tau = t$ and $\frac{\partial X}{\partial \sigma} \cdot \frac{\partial X}{\partial t}$. The first condition is the most natural since it means that a line of constant $\tau$ is a string as seen by a inertial Lorentz observer, and the second condition is chosen such that the $\sigma$ parameter is parallel to the parameter $s$, which is the length along the string parameter and this implies the string velocity coincides with the tranverse velocity, which is a natural dynamical variable of the string since by construction it is reparameterisation invariant, so it doesn't change even tho our action changes under reparameterisation. From this, we find that the new form for the conserved energy per infinitesimal piece of the string is the regular energy of the Nambu-Goto string times the correction factor
...
\begin{equation} \label{eqn:wave-speed-corrected}
	v = \frac{c}{F^+}.
\end{equation}
This is in fact an expected result for a effective theory emerging from a discretised background geometry, see\cite{Bodendorfer-LQG}.
...
\end{multline}
The reason for considering this form and not one in which the inverse metric $g^{a b}$ also gets a correction term is that this form of a Polyakov inverse area-corrected action reduces nicely to the Nambu-Goto corrected action(\ref{eqn:modified-NG-action}) when the auxiliary metric $g_{a b}$ coincides with the induced metric $h_{a b} = \partial _{a} X^{\mu} \partial _{b} X^{\nu} \eta _{\mu \nu}$. This action has the same properties as\eqref{eqn:modified-NG-action}: it is not reparameterisation invariant but is evidently Poincaré-invariant by the same arguments as before.

The equations of motion for the auxiliary metric are completely unchanged by this correction, so they are
...
\begin{multline} \label{eqn:X-eom-inverse-area}
	...
	+ \partial _{a} \left( F^{-} \sqrt{- g} g^{a b} \right) \partial _{b} X^{\nu} \eta _{\lambda \nu} = 0 ,
\end{multline}
where $F^{-} = 1 - k \Delta / 2 (- g)$. In conformal gauge $g^{a b} = (\phi (x))^{-1} \eta ^{a b}$, eq(\ref{eqn:X-eom-inverse-area}) simplify to
...
\end{equation}
The reason we can still use conformal gauge even tho our action no longer has Weyl symmetry is because of a theorem stating that {\it every 2d metric is conformally flat}(theorem 7.2 in\cite{Schottenloher}).

\subsection{Simplifying The Equations of Motion} \label{sec:solving-eoms-inverse-area}

Equation(\ref{eqn:X-eom-simplified}) seems a bit odd, but we can polish it by choosing a plane-wave ansatz for the embedding fields $X^{\mu} = X^{\mu} _{0} \e ^{-i (E \tau - p \sigma)}$ and using it to fix a form for the conformal factor $\phi (x)$ appearing in $F^{-}$. By using the ansatz in eq(\ref{eqn:X-eom-simplified}), it becomes
\begin{equation}
...
\end{equation}
At first glance it seems problematic that we have a complex conformal factor $\phi (x)$, but the complex structure comes from our ansatz for the embedding fields, and when we choose either the real or imaginary part of the latter it will also pick the respective component of $\phi$. With this choice, we thus turned eq(\ref{eqn:X-eom-simplified}) into a Klein-Gordon equation
\begin{equation} \label{eqn:X-Klein-Gordon-eqn}
	\left( \partial _{\sigma} ^2 - \partial _{\tau} ^2 \right) X^{\mu} - \mu ^2 X^{\mu} = 0 .
\end{equation}
The modification to the dispersion relation via the mass term is also an expected effect of a discrete background geometry as mentioned in\cite{Bodendorfer-LQG}.
...
\end{multline}
with $E_{p} = \sqrt{p^2 + \mu ^2}$ for the closed string. From here on, we shall deal only with closed strings, unless otherwise stated. Following an analogous procedure to the one found in \cite{Tong2009Lectures} and \cite{Green2012Superstring}, reality of $X^{\mu}$ implies $(X^{\mu})^{*} = X^{\mu}$, from which we get
...
\end{align}
\begin{equation} \label{eqn:"Virasoro"-modes-sum}
	\sum _{n} \left( \left( \frac{n}{E_{n}} \right) ^2- 1 \right) \widetilde{L}_{n , - n} = 0
\end{equation}

Before proceeding, we have one more remark: the 0-th term of the solution reads
...
\begin{equation} \label{eqn:a-p-relation}
	\alpha ^{\mu} _{0} = 2 \pi \alpha ' \mu x^{\mu} + 4 \pi i (\alpha ')^2 p^{\mu} .
\end{equation}

After analysing closer the $n = - p$ constraint(\ref{eqn:"Virasoro"-modes-sum}), we get the string mass in terms of vibrational modes and center of mass position
\begin{multline} \label{eqn:string-rest-mass}
	M^2 = - p^{\mu} p_{\mu} = \frac{1}{(4 \pi (\alpha ')^2)^2} \Bigg( (2 \pi \alpha ' m)^{2} x^{\mu} x_{\mu} - \\
	- \sum _{n \neq 0} \left( \left( \left( \frac{n}{E_{n}} \right) ^2 - 1 \right) a^{\mu} _{n} (a^{\nu} _{n})^{*} \eta _{\mu \nu} \right) \Bigg) .
\end{multline}

Collecting our results, we have that the solution for our closed string is given by
\begin{multline} \label{eqn:KG-Polyakov-solution-final}
	X^{\mu} (\tau , \sigma) = \frac{1}{2 \pi \alpha '} \sum _{p} \frac{1}{2 E_{p}} \Big( \alpha ^{\mu} _{p} \e ^{i (- E_{p} \tau + p \sigma)} + \\
	+ (\alpha ^{\mu} _{- p})^{*} \e ^{i (E_{p} \tau + p \sigma)} \Big)
\end{multline}
subject to the family of constraints
\begin{equation} \label{eqn:"Virasoro"-constraints}
	L_{n , p} = \widetilde{L}_{n , p} = 0
\end{equation}
\begin{equation*}
	\sum _{n} \left( \left( \frac{n}{E_{n}} \right) ^2- 1 \right) \widetilde{L}_{n , - n} = 0
\end{equation*}
and 0-th oscillator mode related to center of mass position and momentum
\begin{equation*}
	\alpha ^{\mu} _{0} = 2 \pi \alpha ' \mu x^{\mu} + 4 \pi i (\alpha ')^2 p^{\mu} .
\end{equation*}
One thing to note is that the constraint(\ref{eqn:"Virasoro"-modes-2}) implies we only have $D$ non-zero modes in the solution, since all modes must be mutually orthogonal.

\subsection{The Modes and Constraints Algebra} \label{sec:modes-constraint-algebra}

We start by calculating the momentum canonical to $X^{\mu}$, $\Pi _{\lambda}$:
\begin{multline} \label{eqn:canonical-momentum}
	\Pi _{\lambda} = \frac{\partial \Lagr}{\partial (\partial _{\tau} X^{\lambda})} = \\
	= F^{-} \frac{i}{4 \pi \alpha '} \sum _{n \in \mathbb{Z}} \bigg( - \alpha ^{\nu} _{n} \e ^{i (- E_{n} \tau + n \sigma)} + \\
	+ (\alpha ^{\nu} _{- n})^{*} \e ^{i (E_{n} \tau + n \sigma)} \bigg) \eta _{\nu \lambda}
\end{multline}
and proceed by using the canonical equal-time Poisson bracket between $X^{\mu}$ and $\Pi ^{\nu}$
\begin{equation} \label{eqn:equal-time-Poisson-bracket}
	\left\{ X^{\mu} (\tau , \sigma) , \Pi ^{\nu} (\tau , \sigma ') \right\} = \delta (\sigma - \sigma ') \eta ^{\mu \nu}
\end{equation}
which, under the assumption of constant conformal factor $\phi$, leads to
\begin{equation} \label{eqn:modes-bracket-0}
	\left\{ \alpha ^{\mu} _{k} , \alpha ^{\nu} _{- m} \right\} = \left\{ (\alpha ^{\mu} _{- k})^{*} , (\alpha ^{\nu} _{m})^{*} \right\} = 0
\end{equation}
\begin{multline} \label{eqn:modes-bracket-oscillator}
	\left\{ \alpha ^{\mu} _{k} , (\alpha ^{\nu} _{m})^{*} \right\} = - \left\{ (\alpha ^{\mu} _{- k})^{*} , \alpha ^{\nu} _{- m} \right\} = \\
	- i \frac{4 \pi (\alpha ')^{2}}{F^{-}} E_{k} \eta ^{\mu \nu} \delta _{k , m} .
\end{multline}
This are just rescaled harmonic oscillator Poisson bracket relations, so we define the oscillator modes
\begin{align}
	a^{\mu} _{k}       & : = \frac{1}{2 \alpha '} \sqrt{\frac{F^{-}}{\pi}} \frac{1}{\sqrt{E_{k}}} \alpha ^{\mu} _{k} \label{eqn:annihilation-mode}      \\
	(a^{\mu} _{k})^{*} & : =  \frac{1}{2 \alpha '} \sqrt{\frac{F^{-}}{\pi}} \frac{1}{\sqrt{E_{k}}} (\alpha ^{\mu} _{k})^{*} \label{eqn:creation-mode} ,
\end{align}
such that the bracket relations become proper harmonic oscillator bracket relations
\begin{align}
	\left\{ a^{\mu} _{k} , (a^{\nu} _{p})^{*} \right\} & = - i \eta ^{\mu \nu} \delta _{k , p} \label{eqn:oscillator-bracket}                   \\
	\left\{ a^{\mu} _{k} , a^{\nu} _{p} \right\}       & = \left\{ (a^{\mu} _{k})^{*} , (a^{\nu} _{p})^{*} \right\} = 0 \label{eqn:0-bracket} .
\end{align}
By analysing the $k = p = 0$ case, we also get the relation for $x^{\mu}$ and $p^{\nu}$
\begin{align}
	\left\{ x^{\mu} , x^{\nu} \right\} & = \left\{ p^{\mu} , p^{\nu} \right\} = 0 \label{eqn:x-x-p-p-bracket}       \\
	\left\{ x^{\mu} , p^{\nu} \right\} & = \frac{1}{4 \pi \alpha ' F^{-}} \eta ^{\mu \nu} \label{eqn:x-p-bracket} ,
\end{align}
where by choosing
\begin{equation} \label{eqn:constant-conformal-factor}
	\phi = \sqrt{\frac{k \Delta}{2 \left( 1 - \dfrac{1}{4 \pi \alpha '} \right)}}
\end{equation}
we can simplify the bracket(\ref{eqn:x-p-bracket}) to the expected
\begin{equation} \label{eqn:x-p-bracket-normalized}
	\left\{ x^{\mu} , p^{\nu} \right\} = \eta ^{\mu \nu} .
\end{equation}

Having the bracket algebra between the expansion modes and redefining the constraints $L_{n , m}$ and $\widetilde{L}_{k , p}$ to be between the normalized modes $a^{\mu} _{n}$ instead of the $\alpha ^{\mu} _{n}$ (we can do this since $E_{n} = \sqrt{n^2 + \mu ^2} \neq 0$), we get the \textquotedblleft Klein-Gordon-Virasoro algebra"
\begin{align}
	\left\{ L_{n , m} , L_{k , p} \right\}                         & = 0 \label{eqn:L-bracket-0}                                                                                                      \\
	\left\{ L_{n , m} , \widetilde{L}_{k , p} \right\}             & = - i \left( L_{m, k} \delta _{n , - p} + L_{n , k} \delta _{m , - p} \right) \label{eqn:L-bracket-1}                            \\
	\left\{ \widetilde{L}_{n , m} , \widetilde{L}_{k , p} \right\} & = - i \left( \widetilde{L}_{k , m} \delta _{n , - p} - \widetilde{L}_{n , p} \delta _{- m , k} \right) . \label{eqn:L-bracket-2}
\end{align}

\section{The Quantum Klein-Gordon String} \label{sec:quantum-KG-string}

In the following, we proceed to quantize the Klein-Gordon string via canonical covariant quantization, in a manner much similar to those found in \cite{Tong2009Lectures} and \cite{Green2012Superstring}. We turn the Poisson bracket relations into commutation relations between the now operator variables, $[ , ] = i \{ , \}$, yielding
\begin{align}
	\left[ \Hat{X}^{\mu} (\tau , \sigma) , \Hat{\Pi}^{\nu} (\tau , \sigma ') \right] & = i \delta (\sigma - \sigma ') \eta ^{\mu \nu} \label{eqn:commutator-X-Pi}                                      \\
	\left[ \Hat{X}^{\mu} , \Hat{X}^{\nu} \right]                                     & = \left[ \Hat{\Pi}^{\mu} , \Hat{\Pi}^{\nu} \right] = 0 \label{eqn:commutator-X-X-Pi-Pi}                         \\
	\left[ \Hat{a}^{\mu} _{n} , \Hat{a}^{\nu} _{m} \right]                           & = \left[ (\Hat{a}^{\mu} _{n})^{\dagger} , (\Hat{a}^{\nu} _{m})^{\dagger} \right] = 0 \label{eqn:commutator-a-a} \\
	\left[ \Hat{a}^{\mu} _{n} , (\Hat{a}^{\nu} _{m})^{\dagger} \right]               & = \eta ^{\mu \nu} \delta _{n , m} \label{eqn:commutator-a-a*}                                                   \\
	\left[ \Hat{x}^{\mu} , \Hat{x}^{\nu} \right]                                     & = \left[ \Hat{p}^{\mu} , \Hat{p}^{\nu} \right] = 0 \label{eqn:commutator-x-x-p-p}                               \\
	\left[ \Hat{x}^{\mu} , \Hat{p}^{\nu} \right]                                     & = i \eta ^{\mu \nu} . \label{eqn:commutator-x-p}
\end{align}

We define a vacuum state for the string to obey
\begin{equation} \label{eqn:annihilation-vacuum}
	\Hat{a}^{\mu} _{p} \ket{0} = 0 , \ \mathrm{for} \ p \neq 0 .
\end{equation}
Since $\Hat{a}^{\mu} _{0}$ contains $\Hat{x}^{\mu}$ and $\Hat{p}^{\mu}$, the vacuum also has extra structure from those
\begin{align}
	\Hat{x}^{\mu} \ket{0 ; x} & = x^{\mu} \ket{0 ; x} \label{eqn:x-x-rep}                               \\
	\Hat{p}_{\mu} \ket{0 ; x} & = - i \frac{\partial}{\partial x^{\mu}} \ket{0 ; x} \label{eqn:p-x-rep}
\end{align}
in position representation and
\begin{align}
	\Hat{x}^{\mu} \ket{0 ; p} & = - i \frac{\partial}{\partial p_{\mu}} \ket{0 ; p} \label{eqn:x-p-rep} \\
	\Hat{p}_{\mu} \ket{0 ; p} & = p_{\mu} \ket{0 ; p} \label{eqn:p-p-rep}
\end{align}
in momentum representation. Our Fock space is built from the vacuum state $\ket{0}$ by operating with a sequence of creation operators
\begin{multline} \label{eqn:general-state}
	\ket{\psi} = ((\Hat{a}^{\mu _{1}} _{1})^{\dagger})^{n_{\mu _1}} ((\Hat{a}^{\mu _2} _{2})^{\dagger})^{n_{\mu _2}} ... \times \\
	\times ((\Hat{a}^{\nu _1} _{- 1})^{\dagger})^{n_{\nu _1}} ((\Hat{a}^{\mu _2} _{- 2})^{\dagger})^{n_{\nu _2}} ... \ket{0} .
\end{multline}
...
\end{equation}
we have ghosts in the theory which must be dealt with(\cite{Tong2009Lectures},\cite{Green2012Superstring}). When translating the constraints as quantum operators, we require that they have vanishing matrix elements when sandwiched by physical states $\psi$ and $\phi$:
...
\end{equation}
\begin{equation} \label{eqn:L-constranit-quantum-sum}
	\bra{\phi} \sum _{p} \left( \left( \frac{p}{E_{p}} \right) ^2 - 1 \right) \Hat{\widetilde{L}}_{p , - p} \ket{\psi} = 0 .
\end{equation}

When translating the constraints as quantum operators, for $\Hat{L}_{p , k}$ we have no ambiguities since it's composed only of annihilation operators. As for $\Hat{\widetilde{L}}_{p , k}$, there is an ambiguity when $p = - k$, which manifests in the imposition of this constraint as
\begin{equation}
	\bra{\phi} \sum _{p} \left( \left( \left( \frac{p}{E_{p}} \right) ^2 - 1 \right) \Hat{\widetilde{L}}_{p , - p} \right) - a \ket{\psi} = 0 ,
\end{equation}
for some constant $a$\cite{Green2012Superstring}. This constant appears in the mass spectrum of the quantum string as in
\begin{multline}
	\Hat{M}^2 = \frac{1}{(4 \pi (\alpha ')^2)^2} \Bigg( (2 \pi \alpha ' \mu)^{2} \Hat{x}^{\mu} \Hat{x}_{\mu} - \\
	- \left( \sum _{n \neq 0} \left( \left( \frac{n}{E_{n}} \right) ^2 - 1 \right) (\Hat{a}^{\mu} _{n})^{\dagger} \Hat{a}^{\nu} _{n} \eta _{\mu \nu} - a \right) \Bigg) .
\end{multline}
The commutation algebra of constraints is inherited by the constraint algebra(\ref{eqn:L-bracket-0},\ref{eqn:L-bracket-1},\ref{eqn:L-bracket-2}), namely
\begin{equation} \label{eqn:L-commutator-0}
	\left[ \Hat{L}_{n , m} , \Hat{L}_{k , p} \right] = 0
\end{equation}
\begin{equation} \label{eqn:L-commutator-1}
	\left[ \Hat{L}_{n , m} , \Hat{\widetilde{L}}_{k , p} \right] = \Hat{L}_{m , k} \delta _{n , - p} + \Hat{L}_{n , k} \delta _{m , - p}
\end{equation}
\begin{equation} \label{eqn:L-commutator-2}
...
\end{equation}
The only commutator with ordering ambiguities is(\ref{eqn:L-commutator-2}) when $m = - k$ and $p = -n$, thus we include anomalous terms\cite{Green2012Superstring}
\begin{multline} \label{eqn:L-commutator-anomalous}
	\left[ \Hat{\widetilde{L}}_{n , m} , \Hat{\widetilde{L}}_{k , p} \right] = \Hat{\widetilde{L}}_{k , m} \delta _{n , - p} - \Hat{\widetilde{L}}_{n , p} \delta _{- m , k} + \\
	+ C_{n} \delta _{n , -p} + D_{k} \delta _{- m , k} .
\end{multline}
By noting that $C_{- n} = - C_{n}$ and $D_{- n} = - D_{n}$ and using the Jacobi identity of the commutator, we conclude that $C_{n} = - D_{n} = C_{1} n$, where we can find the value of $C_{1}$ by calculating the vacuum expectation value of the commutator $[\Hat{\widetilde{L}}_{1 , k} , \Hat{\widetilde{L}}_{k , - 1}]$, which yields that $C_{1} = 0$, thus {\it the quantum algebra of our constraints has no anomalies}. This is a very important result, since it doesn't constrain the dimension of the background manifold to any particular value, so our model works just as well for $D = 4$ as for any other value {\it a priori}.

After analysing the mass-shell condition for the $n$-th excited state $\zeta _{\mu} (\Hat{a}^{\mu} _{n})^{\dagger} \ket{0 ; p}$\cite{Green2012Superstring}, under the assumption that the momentum wave-function is given by [justification needed]
\begin{equation} \label{eqn:p-wave-func}
	\psi (p) = \frac{\alpha '}{\mu} p^{2} ,
\end{equation}
we find that $\zeta _{\mu} p^{\mu} = 0$ and the momentum of this state is given by
\begin{equation} \label{eqn:nth-vector-mode-momentum}
	2 (4 \pi (\alpha ')^2)^2 p^2 = \frac{n^2}{n^2 + \mu ^2} - 1 - a .
\end{equation}
From this, for the absence of ghosts we then require that
\begin{equation} \label{eqn:a-condition-no-ghosts}
	a \geq \frac{d^2}{d^2 + \mu ^2} - 1 .
\end{equation}
In the boundary case, the $d$-th ($d = D - 1$) vector state is massless, while the other spin-1 states are massive and the ground state is a tachyon. Doing a similar analysis for the $d$-th spin-2 state $\zeta _{\mu \nu} (\Hat{a}^{\mu} _{d})^{\dagger} (\Hat{a}^{\nu} _{d})^{\dagger} \ket{0 ; p}$ with $\zeta _{\mu \nu} = \zeta _{\nu \mu}$ and $\zeta _{\ \mu} ^{\mu} = 0$ yields $\zeta _{\mu \nu} p^{\mu} = \zeta _{\mu \nu} p^{\nu} = 0$ and
\begin{equation} \label{eqn:a-conition-massless-spin-2}
	a = 2 \left( \frac{d^{2}}{d^{2} + \mu ^{2}} - 1 \right)
\end{equation}
for it to be massless. However, the quantity in parenthesis is negative, so doubling it makes it less than the minimum condition for the absense of ghosts(\ref{eqn:a-condition-no-ghosts}), thus we can't get massless spin-2 states. This is not really a problem because as pointed out in\cite{Bodendorfer-LQG}, a discrete background geometry modifies the dispersion relation at the effective level, so even tho at the fundamental level gravity would travel at $c$, in the effective limit it would appear \textquotedblleft massive", however with a very small mass. Thus, since we need $a$ to be at least as big as in(\ref{eqn:a-condition-no-ghosts}), we can choose $a > 0$ to also get rid of the ground state tachyon without the need to introduce supersymmetry or other unverified hypothesis.

The most natural value for $a$ is $a = 1$, which comes from the analysis of physical and spurious states\cite{Green2012Superstring} of the form
\begin{equation} \label{eqn:phys-spurious-state}
	\ket{\psi _{- 2 , n}} = \sum _{m} \Hat{\widetilde{L}}_{n , m} \ket{\widetilde{\chi}_{- 2 , m}} ,
\end{equation}
where $\ket{\widetilde{\chi}_{- 2 , m}}$ satisfies the constraints(\ref{eqn:L-constraint-quantum-1},\ref{eqn:L-constraint-quantum-2}) and also
\begin{multline} \label{eqn:spurious-basis-condition}
	\Bigg( \sum _{n} \left( \left( \left( \frac{n}{E_{n}} \right) ^2 - 1 \right) \Hat{\widetilde{L}}_{n , - n} \right) - \\
	- a + 2 - m \Bigg) \ket{\widetilde{\chi}_{- 2 , m}} = 0 .
\end{multline}
States of the form(\ref{eqn:phys-spurious-state}) are annihilated by $\Hat{\widetilde{L}}_{p , k}$ for $p \neq - k$, and for these cases we have
\begin{equation*}
	\sum _{n} \left( \left( \left( \frac{n}{E_{n}} \right) ^2 - 1 \right) \Hat{\widetilde{L}}_{- 1 , - n} \ket{\psi _{- 2 , n}} \right) =
\end{equation*}
\begin{multline*}
	= \sum _{n} \Bigg( \left( \left( \frac{n}{E_{n}} \right) ^2 - 1 \right) \Hat{\widetilde{L}}_{- 1 , - n} \times \\
	\times \sum _{m} \Hat{\widetilde{L}}_{n , m} \ket{\widetilde{\chi}_{- 2 , m}} \Bigg)  =
\end{multline*}
\begin{equation} \label{eqn:spurious-annihilation}
...
\begin{multline} \label{eqn:bimetric-Polyakov}
	S_{BP} = -\frac{T}{2} \int \diff^2 x\sqrt{-g} g^{ab} \partial_a X^\mu \partial_b X^\nu G_{\mu\nu} - \\
	-\frac{T'}{2} \int \diff^2 x \sqrt{-h} h^{ab} \partial_a X^\mu \partial_b X^\nu H_{\mu\nu} + \\
	+ \int \diff ^2 x \Phi (g , h) \Psi (X) \Theta (G , H) ,
\end{multline}
where the last term defines an interaction term $S_{int}$.

\subsection{The Bimetric Polyakov Equations} \label{sec:bimetric-polyakov-eoms}

Following the usual procedure such as the one found in \cite{Tong2009Lectures} with the bulk metric fixed, we find two copies of the string energy-momentum tensor being set to zero, which can be used to find an expression for the auxiliary metrics $g_{c d}$ and $h_{c d}$:
\begin{equation} \label{eqn:variation-wrt-g}
	\frac{\delta S_{B P}}{\delta g^{c d}} = 0
\end{equation}
\begin{multline} \label{eqn:g-em-tensor}
	T^{(G)} _{c d} := G_{\mu \nu} \bigg( \partial _{c} X^{\mu} \partial _{d} X^{\nu} - \\
	- \frac{1}{2} g_{c d} g^{a b} \partial _{a} X^{\mu} \partial _{b} X^{\nu} \bigg) = \frac{2}{T \sqrt{- g}} \frac{\delta S_{int}}{\delta g^{c d}}
\end{multline}

\begin{equation} \label{eqn:g-expression}
	g_{c d} = 2 f^{(G)} \left( \partial _{c} X^{\mu} \partial _{d} X^{\nu} G_{\mu \nu} - \frac{2}{T \sqrt{- g}} \frac{\delta S_{int}}{\delta g^{c d}} \right) ,
\end{equation}
...
\begin{equation} \label{eqn:variation-wrt-h}
	\frac{\delta S_{B P}}{\delta h^{c d}} = 0
\end{equation}
\begin{multline} \label{eqn:h-em-tensor}
	T^{(H)} _{c d} := H_{\mu \nu} \bigg( \partial _{c} X^{\mu} \partial _{d} X^{\mu} - \\
	- \frac{1}{2} h_{c d} h^{a b} \partial _{a} X^{\mu} \partial _{b} X^{\nu} \bigg) = \frac{2}{T' \sqrt{- h}} \frac{\delta S_{int}}{\delta h^{c d}}
\end{multline}
\begin{equation} \label{eqn:h-expression}
	h_{c d} = 2 f^{(H)} \left( \partial _{c} X^{\mu} \partial _{d} X^{\nu} H_{\mu \nu} - \frac{2}{T' \sqrt{- h}} \frac{\delta S_{int}}{\delta h^{c d}} \right) ,
\end{equation}
with
\begin{equation} \label{eqn:h's-f-function}
	\frac{1}{f^{(H)}} = h^{a b} \partial _{a} X^{\mu} \partial _{b} X^{\nu} H_{\mu \nu} .
\end{equation}
If $S_{int} = 0$ and the auxiliary metrics $g_{a b}$ and $h_{a b}$ coincide with the induced metrics from $G_{\mu \nu}$ and $H_{\mu \nu}$, then the action\eqref{eqn:bimetric-Polyakov} reduces to
\begin{equation} \label{eqn:reduced-bimetric-action}
	S' _{B P} = - T \int \diff ^2 x \sqrt{- g} - T' \int \diff ^2 x \sqrt{- h} ,
\end{equation}
which, if $T' = - T k \Delta / 2$ and $h = g^{-1}$ becomes precisely the Nambu-Goto area corrected action\eqref{eqn:modified-NG-action}. The condition $h = g^{-1}$ can be achieved by
\begin{equation} \label{eqn:h=SgS}
	h_{a b} = S_{a c} g^{c d} S_{d b} ,
\end{equation}
where $S \in \mathrm{SL}(2 , \mathbb{C})$. The fact that we require $S = S^{\dagger}$ for $h$ to be real and symmetric restricts $S$ to be in the boosts sector of $\mathrm{SL}(2 , \mathbb{C})$. By requiring that the induced metric by $H_{\mu \nu}$ be equal to\eqref{eqn:h=SgS}, we get
\begin{equation} \label{eqn:H-from-SgS}
	\partial _{a} X^{\mu} \partial _{b} X^{\nu} H_{\mu \nu} = S_{a c} g^{c d} S_{d b}
\end{equation}
from which follows
\begin{equation}
	H_{\mu \nu} = \frac{1}{8 (- g)} (\Tr (S \varepsilon))^2 G_{\mu \nu} ,
\end{equation}
constraining $S$ to necessarily being not symmetric otherwise the trace $\Tr (S \varepsilon)$ vanishes and we would get $H _{\mu \nu} = 0$. In fact, to recover the modified Nambu-Goto action\eqref{eqn:modified-NG-action}, we require $(\Tr (S \varepsilon))^2 = 8$, thus $H_{\mu \nu} = G_{\mu \nu} / (- g)$.
...
\begin{equation} \label{eqn:variation-wrt-X}
	\frac{\delta S_{P B}}{\delta X^{\lambda}} = 0
\end{equation}
\begin{equation} \label{eqn:bimetric-X-eom}
	\partial _{a} \left( F^{a b} _{\mu \lambda} \partial _{b} X^{\mu} \right) = \frac{1}{2} \partial _{\lambda} F^{a b} _{\mu \nu} \partial _{a} X^{\mu} \partial _{b} X^{\nu} ,
\end{equation}
where the coupled double-metric $F^{a b} _{\mu \nu}$ is given by
\begin{equation} \label{eqn:coupled-double-metric}
	F^{a b} _{\mu \nu} = T \sqrt{- g} g^{a b} G_{\mu \nu} + T' \sqrt{- h} h^{a b} H_{\mu \nu} .
\end{equation}
If the conditions for $h = g^{-1}$ are satisfied and $G_{\mu \nu} = \eta _{\mu \nu}$, eq\eqref{eqn:bimetric-X-eom} reduces to
\begin{multline} \label{eqn:bimetric-X-simplified-1}
	\partial _{a} \big( \big( \sqrt{- g} g^{a b} - \\
	- \frac{k \Delta}{2 (- g)} \sqrt{- g}^{-1} (S^{-1})^{a c} g_{c d} (S^{-1})^{d b} \big) \partial _{b} X^{\mu} \big) = 0 ,
\end{multline}
which in conformal gauge $g_{a b} = \phi (x) \eta _{a b}$ and with
\begin{equation} \label{eqn:S-simplification-condition}
	(S^{-1})^{a c} \eta _{c d} (S^{-1})^{d b} = \eta ^{a b}
\end{equation}
simplifies further to
\begin{equation} \label{eqn:bimetric-eom-simplified}
	\eta ^{a b} \partial _{a} \partial _{b} X^{\mu} + \eta ^{a b} \partial _{a} \ln (F^{-}) \partial _{b} X^{\mu} = 0 .
\end{equation}
This is the exact form derived earlier in\eqref{eqn:X-eom-simplified}, thus leading to the same Klein-Gordon equation.
