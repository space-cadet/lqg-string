...
Variation of the modified Polyakov action(\ref{eqn:lqg-string-action}) w.r.t the zweibein $e^{c} _{k}$ yields the vanishing of the Lie algebra-valued energy-momentum tensor, which can be contracted with the dyad to yield the regular stress-energy tensor:
\begin{equation} \label{eqn:variation-wrt-e}
	\frac{\delta S_{PA}}{\delta e^{c} _{k}} \overset{!}{=} 0
\end{equation}
\begin{equation*}
	\downarrow
\end{equation*}
\begin{equation} \label{eqn:vanishing-em-tensor}
	T^{k} _{c} := E^{I} _{\mu} E_{\nu I} (e^{a k} D_{a} \Tilde{X}^{\mu} D_{c} \Tilde{X}^{\nu} -
\end{equation}
\begin{equation*}
	- e^{k} _{c} e^{a} _{i} e^{b i} D_{a} \Tilde{X}^{\mu} D_{b} \Tilde{X}^{\nu}) = 0 .
\end{equation*}
Equation(\ref{eqn:vanishing-em-tensor}) can be used to obtain an expression for the dyads in terms of the string coordinates $\Tilde{X}^{\mu}$ as in
\begin{equation} \label{eqn:dyad-string-coordinates}
	e^{k} _{c} = F e^{a k} D_{a} \Tilde{X}^{\mu} D_{c} \Tilde{X}^{\nu} E^{I} _{\mu} E_{\nu I} ,
\end{equation}
where the function $F$ is given by
\begin{equation} \label{eqn:F-function-dyad}
	\frac{1}{F} = e^{a} _{i} e^{b i} D_{a} \Tilde{X}^{\mu} D_{b} \Tilde{X}^{\nu} E^{I} _{\mu} E_{\nu I} .
\end{equation}
