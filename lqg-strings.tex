...
\begin{equation} \label{eqn:h-delta-relation}
...
\begin{multline} \label{eqn:SgS-implication}
	\left( f^{(H)} \delta _{a} ^{c'} \delta _{b} ^{d'} H_{\mu \nu} - \frac{1}{2 g} f^{(G)} \Delta _{a c} \varepsilon ^{c c'} \Delta _{b d} \varepsilon ^{d d'} G_{\mu \nu} \right) \times \\
...
\begin{equation} \label{eqn:delta-epsilon-relation}
...
\begin{equation} \label{eqn:H-G-relation}
...
\end{equation}
If we impose the conditions \eqref{eqn:h-delta-relation}, \eqref{eqn:delta-epsilon-relation} and \eqref{eqn:H-G-relation} plus $T' = - T k / 2$ in the interactionless bimetric Polyakov action \eqref{eqn:polyakov-bimetric}, we obtain the Polyakov version of the area-corrected action \eqref{eqn:modified-ng-action}:
...
If one were to calculate the equations of motion for the quantum of area field $\Delta _{a b}$ in the actions \eqref{eqn:modified-ng-action} and \eqref{eqn:inverse-metric-Polyakov} explored before, one would end up concluding that either $\Delta _{a b} = 0$ or it's determinant $\Delta = 0$ vanish. This happens because, as we explored in section \ref{sec:ws-kalb-ramond}, $\Delta _{a b}$ should be related to the pullback of the Kalb-Ramond field $B_{\mu \nu}$, but the actions discussed were actions for the free string, not coupled to said field. The more complete picture would be thus to consider the (area-corrected) string now coupled to the Kalb-Ramond field \cite{Tong2009Lectures}, since this represents the more general and complete diffeomorphism invariant action in $1+1$ dimensions:
...
\end{equation}
from which we can find the pullback of the Kalb-Ramond field $b_{a b} := \del _{a} X^{\mu} \del _{b} X^{\nu} B_{\mu \nu}$ given in terms of the worldsheet metric $g$ and the quantum of area density $\Delta$ 
...
To see how the area 2-form $\Delta _{ab}$ is related to the Kalb-Ramond field we have to go back to \autoref{sec:ws-kalb-ramond} where we looked at the case of the bimetric string action \eqref{eqn:polyakov-bimetric}. We saw that the bimetric Polyakov action can be expressed, quite obviously, as a bimetric Nambu-Goto action \eqref{eqn:double-ng-action}, which we reproduce below for convenience:
\begin{equation}
	S = - T \int \diff ^2 x \sqrt{- g} - T' \int \diff ^2 x \sqrt{- h}.
\end{equation}
From here if we make the mapping $T' = - T k / 2$ and $h = \Delta ^2 / g$, we recover the Nambu-Goto area-corrected action \eqref{eqn:modified-ng-action}
\begin{equation}
	S = - T \int \diff ^2 x \left( \sqrt{- g} - \frac{1}{2} \frac{k \Delta}{\sqrt{- g}} \right) .
\end{equation}
Now, as noted in the main text, the condition $h = \Delta^2/g$ can be achieved if the following holds true \eqref{eqn:h-delta-relation}:
\begin{equation*}
	h_{ab} = \Delta_{ac} \Delta_{bd} g^{cd},
\end{equation*}
where $g^{cd}$ is the inverse of $g_{ab}$. Now, let us assume that $\Delta_{ab}$ can be written as \eqref{eqn:delta-epsilon-relation}:
\begin{equation*}
	\Delta_{ab} = \sqrt{\Delta} \epsilon_{ab}.
\end{equation*}
Now, we recall that the inverse of a $2\times 2$ matrix can be written using the total-antisymmetric symbol as:
\begin{equation}
	g^{ab} = \frac{1}{2g} \epsilon^{ac} \epsilon^{bd} g_{cd},
\end{equation}
where $g = \det{g_{ab}}$. Inserting this identity into \eqref{eqn:h-delta-relation}, and using the expression for the the determinant of a matrix in terms of product of epsilon symbols:
\begin{equation*}
