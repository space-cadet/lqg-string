...
\end{equation}
and in the special case of the second metric coming from the quantum geometry inverse area correction where $T' = T g \Delta / 2$ and the background geometry being fixed i.e $\partial _{\lambda} F_{\mu \nu} = 0$, we see that eq(\ref{eqn:bimetric-wave-equation}) almost reduces to eq(\ref{eqn:simplified-modified-wave-eqn}), the only difference being that the correction factor loses the dependence on the determinant $h$.
