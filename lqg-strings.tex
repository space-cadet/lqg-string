...
\end{multline}
where $F^{-} = 1 - k \Delta / 2 (- g)$. Using conformal gauge $g^{a b} = (\phi (x))^{-1} \eta ^{a b}$ and assuming flat background $G_{\mu \nu} = \eta _{\mu \nu}$, eq(\ref{eqn:X-eom-inverse-area}) simplify to
...
\end{equation}
from which we can use the freedom in the conformal function to fix the second term as $- \mu ^2 X^{\mu}$, with $- \mu ^2 = p^2 - E^2$ on-shell, and this in turn implies that the derivatives of $\ln (F^{-})$ are given by
...
In the following, we proceed to quantize the Klein-Gordon string via canonical covariant quantization, in a manner much similar to those found in \cite{Tong2009Lectures} and \cite{Green_Schwarz_Witten_2012}. To start, we calculate the momentum canonical to the embbeding fields $X^{\mu}$, which reads
\begin{multline} \label{eqn:canonical-momentum}
	\Pi _{\mu} (\tau , \sigma) = F^{-} \frac{i}{(4 \pi \alpha ')^2} \sum _{p} \left( - \alpha ^{\nu} _{p} \e ^{i (- E_{p} \tau + p \sigma)} + \\
...
\begin{equation} \label{eqn:alternative-classical-rest-mass}
	M^2 = - \frac{1}{(4 \pi (\alpha ')^2)^2} \lim _{p \rightarrow 0} \left( - \frac{E_{p}}{p} \widetilde{L}_{p , 0} \right) ,
\end{equation}
