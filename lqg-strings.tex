...
\end{equation}
Presumably the limit $k \ll 1$ corresponds to when the area of the string worldsheet is large compared to $\Delta$, $h \gg \Delta$. Thus in this limit $ k^2 \ll 1$ and $ h^2 \gg 1$ because of which we can drop the $\mc{O}(k^2/h^2)$ corrections. The final area corrected Nambu-Goto action becomes:
...
However, diffeomorphisms and gauge transformations are not independent of each other. A diffeomorphism is a differentiable map from one manifold to another (in the case of general relativity both manifolds are generally taken to be the same), which has a differentiable (and invertible) map. In more pedestrian language, a diffeomorphism is an (invertible) change of co-ordinates on a manifold. A gauge transformation is a redefinition of the local fields which live on the underlying manifold. In fact, at least for certain geometries, these two can be seen to be equivalent to each other, with diffeomorphisms representing ``passive'' co-ordinate transformations and gauge representing ``active'' co-ordinate transformations.
...
\subsection{Is the Area-Corrected Nambu-Goto Action a Born-Infeld Model?} \label{sec:area-corrected-BI}

The Nambu-Goto area-corrected action before approximations \eqref{eqn:NG-area-corrected} has a structure strikingly similar to the Born-Infeld model
...
\end{equation}
thus the action \eqref{eqn:NG-area-corrected} can be recast as a Born-Infeld model for the string with $k$ playing the role of inverse tension squared and $\Delta _{a b}$ the role of the pullback of a field strength. Since we explored the relation between $\Delta _{a b}$ and the Kalb-Ramond field $B_{\mu \nu}$ in \autoref{sec:WS-Kalb-Ramond-QoA}, this could mean that the Kalb-Ramond 2-form is exact, $B = \diff A$, and thus its exterior derivative, which gives the torsion of the background spacetime \cite{Tong2009Lectures}, vanishes $\diff B = \diff (\diff A) = 0$. Also, we have a direct relation between string tension and scale
\begin{equation} \label{eqn:k-T}
	\frac{l_p}{l_s} = k \sim (2 \pi \alpha ')^2 = \frac{1}{T^2} .
\end{equation}
