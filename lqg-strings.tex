...
$X^\mu(\sigma, \tau)$ are the space-time coordinates of a point $(\sigma, \tau)$ on the worldsheet. Now if one takes the sigma model viewpoint then the $X$'s can be viewed as $D+1$ scalar fields living on a $1+1$ worldsheet.
But these are not scalars, they are components of a $D+1$ dim vector because they transform as elements of $SO(D,1)$ under global Lorentz transformations on the ambient space-time.

The key point here is that the Lorentz transformations acting on the $X$'s are \textbf{global}. If the background geometry is taken to be Minkowski then the $X$'s become points on this manifold and $SO(D,1)$ just takes one point in Minkowski to another point in Minkowski. The overall geometry is invariant under SO(D,1). 
Now, consider the case when the background geometry is not flat. Immediately you can see that the $X$'s can no longer be treated as elements of a $SO(D,1)$ vector under \emph{global} Lorentz transformations.

But now instead of viewing the $X$'s as elements of a Minkowski space, we think of them as elements of the tangent space $T_p(M)$ of the background manifold $M$.
And what happens when you go from point $p$ to point $p'$ in $M$, the local Lorentz frames at the two points are related by an element of $g(p,p') \in SO(D,1)$. This group element can be thought of arising from taking the holonomy of a $\mf{so}(D,1)$ Lie algebra valued connection living on $M$, along a path connecting $p$ and $p'$.

Now, let's remind ourselves what we're trying to do here. We want a gauge field, which lives on the background geometry and which encodes the background geometry, should appear in an action of a Bosonic string. The end result is that we now have with a gauge field $\mc{A}_a^J(\sigma, \tau)$ living on the worldsheet $\Sigma$. Here the lower index $a$ is the worldsheet index and $J$ labels generators of the Lie algebra $\mf{so}(D,1)$. Remember this is a gauge field which is now living on the worldsheet. The consequence for the string action is that in order for the action to be invariant under local gauge transformations of $\mc{A}$, we have to replace the ordinary derivatives of $X^\mu$ with the gauge covariant derivative $\partial_a X^\mu \rightarrow \mc{D}_a X^\mu$, which is defined as:

\begin{equation}
	\mc{D}_a X^I(\sigma,\tau) = \partial_a X^I + g \mc{A}_a X^I,
\end{equation}
where $\mc{A}_a$ is a field living on $\Sigma$ which takes values in the Lie algebra $\mf{so}(D,1)$.
...
\section{Worldsheet Action in Terms of Transverse Velocity}
