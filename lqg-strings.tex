...
\end{equation}
In the special case where the second metric comes from the quantum geometry correction in the form of inverse area, where $T' = T k \Delta / 2$, eq(\ref{eqn:bimetric-wave-equation}) thus reduces to
\begin{equation} \label{eqn:reduced-Polyakov-equation}
...
\end{equation}
where if the R.H.S is set to be proportional to the embbeding fields, we thus get a Klein-Gordon equation
\begin{equation} \label{eqn:Polyakov-Klein-Gordon-equation}
	\eta ^{a b} \partial _{a} \partial _{b} X^{\mu} - \kappa X^{\mu} = 0 .
\end{equation}

\subsection{Relation With The Nambu-Goto Equation}

At first glance, our recent analysis seems to conflict with the one done in sec(\ref{sec:modified-eom}), but as pointed out in \cite{}, the variable-speed wave equation can be turned into a Klein-Gordon equation as long as the speed function satisfies certain constraints [need to show], while it is known from the GR literature that variable-speed waves are equivalent to waves in curved space, which is precisely what we found, since for eq(\ref{eqn:bimetric-wave-equation}) to reduce into the Klein-Gordon equation(\ref{eqn:Polyakov-Klein-Gordon-equation}) we needed for the background geometry to be dynamical.
