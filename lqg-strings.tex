...
Although the action\eqref{eqn:modified-NG-action} is clearly not reparameterisation invariant anymore since the second term has a inverse metric determinant, thus under reparameterisation would pick up a squared Jacobian, it is still manifestly Poincaré invariant, since it only contains derivatives of $X^{\mu}$, thus it's variations vanish under global Lorentz transformations and translations. At face value, this action does not appear to satisfy diffeomorphism invariance of the string worldsheet because of the second term. This stems from the fact that we are assuming the existence of a minimum quantum of area, so it is obvious that diffeomorphism invariance, at least in the classical sense, will no longer be an exact symmetry of the theory.
...
\end{multline}
where $F^{-} = 1 - k \Delta / 2 (- g)$. In conformal gauge $g^{a b} = (\phi (x))^{-1} \eta ^{a b}$ and assuming flat background $G_{\mu \nu} = \eta _{\mu \nu}$, eq(\ref{eqn:X-eom-inverse-area}) simplify to
