...
\begin{equation} \label{variation-wrt-h}
	\frac{\delta S_{P B}}{\delta h^{c d}} \overset{!}{=} 0
\end{equation}
\begin{equation*}
	\downarrow
\end{equation*}
\begin{multline} \label{h-em-tensor}
...
\begin{equation*}
	\Downarrow
\end{equation*}
\begin{equation} \label{h-expression}
	h_{c d} = 2 f^{(H)} \partial _{c} X^{\mu} \partial _{d} X^{\nu} H_{\mu \nu} ,
\end{equation}
with
\begin{equation} \label{h's-f-function}
	\frac{1}{f^{(H)}} = h^{a b} \partial _{a} X^{\mu} \partial _{b} X^{\nu} H_{\mu \nu} .
\end{equation}

As for the embbeding fields equation, we get a wave equation for the combined metric
