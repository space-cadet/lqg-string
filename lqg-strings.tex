...
\begin{equation} \label{eqn:bimetric-wave-equation}
	\partial _{a} \left( f^{\prime a b} \partial _{b} X^{\mu} \right) F_{\mu \lambda} = 0 ,
\end{equation}
with the coupled metric $f^{\prime a b}$ and the combined bulk metric $F_{\mu \nu}$ given by
...
\begin{equation} \label{eqn:combined-bulk-metric}
	F_{\mu \nu} = G_{\mu \nu} + H_{\mu \nu},
\end{equation}
where the coupled metric $f^{\prime a b}$ has a prime since it is not quite the metric induced by the combined metric $F_{\mu \nu}$. In the special case where $h = g^{-1}$ and $T' = T k \Delta / 2$, we get
\begin{multline} \label{eqn:h-equal-g-inverse-f'}
	f^{\prime a b} = T \bigg( \sqrt{- g} g^{a b} + \\
	+ \frac{k \Delta}{2} \sqrt{- g^{-1}} U^{a} _{\ a'} (g^{-1})^{a' b'} U^{\ b} _{b'} \bigg) ,
\end{multline}
where we have a new $\mathrm{SO}(1,1)$ symmetry in the second metric since forcing the determinant to be the inverse of the first doesn't mean the second matrix will be just the inverse, but rather any matrix that differs by an orthogonal transformation $U$. Given the metric itself is a function of world-sheet coordinates, we require this to be a local symmetry $U(x)$, thus eq(\ref{eqn:bimetric-wave-equation}) becomes
\begin{multline} \label{eqn:gauge-wave-eqn}
	f^{\prime a b} \partial _{a} \partial _{b} X^{\mu} + T \Big( \partial _{a} (\sqrt{- g} g^{a b}) + \\
	+ \frac{k \Delta}{2} \Big( U^{a} _{\ a'} \partial _{a} (\sqrt{- g^{-1}} (g^{-1})^{a' b'}) U^{\ b} _{b'} + \\
	+ \partial _{a} U^{a} _{\ a'} (\sqrt{- g^{-1}} (g^{-1})^{a' b'}) U^{\ b} _{b'} + \\
	+ U^{a} _{\ a'} (\sqrt{- g^{-1}} (g^{-1})^{a' b'}) \partial _{a} U^{\ b} _{b'} \Big) \Big) \partial _{b} X^{\mu} = 0 ,
\end{multline}
and after applying the conformal symmetry of 2D metrics in the form $g^{a b} = (\phi (x))^{-1} \eta ^{a b}$ the above equation reduces to
\begin{multline}
	\left( 1 + \frac{k \Delta}{2} \right) \eta ^{a b} \partial _{a} \partial _{b} X^{\mu} + \\
	+ \frac{k \Delta}{2} \Big( \partial _{a} U^{a} _{\ a'} \eta ^{a' b'} U^{\ b} _{b'} + \\
	+ U^{a} _{\ a'} \eta ^{a' b'} \partial _{a} U^{\ b} _{b'} \Big) \partial _{b} X^{\mu} = 0 .
\end{multline}
