...
The actions \eqref{eqn:modified-NG-action} and \eqref{eqn:inverse-metric-Polyakov} used in sections \ref{sec:modified-ng} and \ref{sec:inverse-metric-Polyakov} have a glaring disadvantage: they do not have WS diffeomorphism invariance, making it so it is not clear what WS parameterization should be used and neither if physical predictions can be extracted from them since it is not obvious that different parameterizations describe the same system. If we want to formulate an effective stringy action that takes LQG quantum geometric effects into consideration and retains WS diffeomorphism invariance while giving similar results to the analysis done in section \ref{sec:inverse-metric-Polyakov}, we arrive at a similar model as \cite{Afshordi-emergent}, with the action being
