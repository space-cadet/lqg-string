...
\begin{equation} \label{eqn:H=1/(2g)G}
	H_{\mu \nu} = \frac{f^{(G)}}{f^{(H)}} \frac{\Delta}{2 g} G_{\mu \nu} .
\end{equation}
...
We start from the action in \eqref{eqn:area-corrected-K-R-action-v2} which describes the area-corrected string coupled to the Kalb-Ramond field, since this is the most generic and complete 2-dimensional action with worldsheet diffeomorphism invariance and bulk global Lorentz symmetry, with the correction to the worldsheet geometry due to quantum geometry proposed by our central dogma. The Levi-Civita tensor $\varepsilon ^{a b}$ has been written in terms of the quantum of area 2-form $\Delta _{a b}$ by dividing and multiplying by $\sqrt{\Delta}$. Varying this action \wrt \ $\Delta ^{c d}$ yields
