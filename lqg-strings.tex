...
\begin{equation} \label{eqn:expansion-modes-bracket-1}
	\left\{ \alpha ^{\mu} _{n} , (\alpha ^{\nu} _{p})^{*} \right\} = - i E_{m} \delta _{|m| , |p|} \eta ^{\mu \nu} = - \left\{ (\alpha ^{\mu} _{n})^{*} , \alpha ^{\nu} _{p} \right\} ,
\end{equation}
...
\begin{equation} \label{eqn:ground-state-mass-shell}
	\frac{\alpha '}{8} p^2 = - \left( \frac{2 \mu ^2}{\alpha '} \left| \frac{\partial \psi (p)}{\partial p} \right| ^2 + a \right) .
\end{equation}
Next we look at the $n$-th excited state $\zeta _{\mu} (\Hat{a}^{\mu} _{d})^{\dagger}$ with $\zeta _{\mu} = \zeta _{\mu} (p)$ being the polarization covector. The mass-shell now reads
\begin{multline} \label{eqn:nth-state-mass-shell}
	...
	- 4 \sinh ^2 \left( \frac{1}{4} \ln (n^2 + \mu ^2) \right) + a \Bigg) ,
\end{multline}
...
\begin{equation} \label{eqn:p-wave-function}
	\psi (p) = - \frac{\alpha '}{8 \mu} p^{\mu} p_{\mu} ,
\end{equation}
...
\begin{equation} \label{eqn:nth-mass-shell-simplified}
	\frac{\alpha '}{4} p^2 = 4 \sinh ^2 \left( \frac{1}{4} \ln (n^2 + \mu ^2) \right) - a
\end{equation}
for the $n$-th state. The norm of these states is given by $\zeta _{\mu} \zeta ^{\mu}$, so if we choose $p^{\mu}$ to lie on the $(0 , 1)$-plane, then the $D - 2$ spacelike polarizations normal to said plane have positive norm. If $a$ is chosen such that the $n = d , d = D - 1$ excited state is a tachyon with $p^2 > 0$, then $p^{\mu}$ can be taken to have no time component, leaving $\zeta _{\mu}$ timelike with negative norm. If the choice is made such that $p^2 < 0$, $p^{\mu}$ can be chosen to have only a time component, making $\zeta _{\mu}$ spacelike with positive norm again. At last, if we choose for $p^2 = 0$, then $\zeta _{\mu}$ is proportional to $p^{\mu}$ with 0 norm, so for the absence of ghosts we require that
\begin{equation} \label{eqn:c-no-ghost-spin-1}
	a \geq 4 \sinh ^2 \left( \frac{1}{4} \ln (d^2 + \mu ^2) \right) .
\end{equation}
In the boundary case, the $d$-th excited vector state is massless and all other vector states (plus the ground state) are regular massive particles with $M^2 > 0$. This, however, is to get a massless vector state for the closed string. The actual massless particle we want from the closed string is a symmetric, traceless spin-2 particle, so let's now consider the state $\zeta _{\mu \nu} (\Hat{a}^{\mu} _{d})^{\dagger} (\Hat{a}^{\nu} _{d})^{\dagger} \ket{0 ; p}$ where $\zeta _{\mu \nu}$ is the polarization tensor. The mass-shell reads
\begin{equation} \label{eqn:dth-spin-2-mass-shell}
	\frac{\alpha '}{4} p^2 = 8 \sinh ^2 \left( \frac{1}{4} \ln (d^2 + \mu ^2) \right) - a
\end{equation}
and the auxiliary $\bra{0 ; p} \Hat{\widetilde{L}}_{d , 0} \zeta _{\mu \nu} (\Hat{a}^{\mu} _{d})^{\dagger} (\Hat{a}^{\nu} _{d})^{\dagger} \ket{0 ; p}$ together with the momentum wave-function \eqref{eqn:p-wave-function} implies
\begin{equation}
	\zeta _{\mu \nu} p^{\mu} = 0 .
\end{equation}
We can decompose the polarization into a symmetric traceless part $G_{\mu \nu} = G_{\nu \mu} , \eta ^{\mu \nu} G_{\mu \nu} = 0$, a anti-symmetric part $B_{\mu \nu} = - B_{\nu \mu}$ and a trace part $\Phi \eta _{\mu \nu} , \Phi = \eta ^{\alpha \beta} \zeta _{\alpha \beta} / D$, $\zeta _{\mu \nu} = G_{\mu \nu} + B_{\mu \nu} + \Phi \eta _{\mu \nu}$, giving rise to 3 well-knwon types of background fields: the graviton $G_{\mu \nu}$, Kalb-Ramond 2-form $B_{\mu \nu}$ and dilaton $\Phi$.
