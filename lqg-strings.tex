...
\end{equation}
where $X^\mu = \{X^0, \vec X\}$ are the fields we had originally and $X^r$ is our new ``dummy'' co-ordinate.

treating the $X$'s as co-ordinates for the bulk background and also as
...
\end{multline}
where $\nabla _{a}$ denotes the 2-dimensional covariant derivative with respect to the Levi-Civita connection $\Gamma _{a b} ^{c}$.
...
\end{equation}
which makes sense since $\Delta _{a b}$ is related to area, so it should be a 2-form. Since $g_{a b}$ is taken to be the pullback of the background metric $G_{\mu \nu}$, $g_{a b} = \del _{a} X^{\mu} \del _{b} X^{\nu} G_{\mu \nu}$, which is a part of the spin-2 spectrum of the closed string, perhaps the quantum area correction field $\Delta _{a b}$ is related to the pullback of the background Kalb-Rammond 2-form, which is the other half of the first excitation of the closed spin-2 spectrum,
\begin{equation} \label{eqn:Delta-B-relation}
	\Delta _{a b} \sim \del _{a} X^{\mu} \del _{b} X^{\nu} B_{\mu \nu} ,
\end{equation}
thus giving a concrete physical interpretation for the $B_{\mu \nu}$ field as being related directly to quantum geometry.
