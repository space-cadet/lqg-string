...
%DIF DEL lqg-strings-aop-v1.tex   Sun Jul  6 11:08:34 2025
%DIF ADD lqg-strings.tex          Sun Sep  7 22:16:58 2025
...
%DIF 52c52
%DIF < \usepackage[breaklinks,colorlinks=true,citecolor=blue,linkcolor=blue,final]{hyperref}
%DIF -------
\usepackage[colorlinks=true,citecolor=blue,linkcolor=blue,final]{hyperref} %DIF > 
%DIF -------
%\usepackage{hyperref}
...
%DIF 68-74d68
%DIF < % Biblatex setup
%DIF < % The default is the 'numerical' style.
%DIF < %\usepackage[backend=biber,url=true,eprint=true,doi=true,sorting=none,backref=true,style=phys]{biblatex}
%DIF < % We use the following bibliography databases:
%DIF < %\usepackage{bibtex}
%DIF < %\addbibresource{../bib_library.bib}
%DIF < % End biblatex
%DIF -------
...
%DIF -------
\newcommand{\ie}{\textit{i.e.}} %DIF > 
\newcommand{\eg}{\textit{e.g.}} %DIF > 
\newcommand{\wrt}{\textit{w.r.t.}} %DIF > 
\newcommand{\etc}{\textit{etc.}} %DIF > 
%DIF -------
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \affiliation{502, Broadway Apartments, Mangaluru, Karnataka, India}
\DIFaddend \author{\DIFdelbegin \DIFdel{Luigi }\DIFdelend \DIFaddbegin \DIFadd{Lin }\DIFaddend Teixeira de Sousa}
...
The following is the outline of the paper along with a brief summary of what is accomplished in each section. For those seeking a rapid review of the principle areas of \DIFdelbegin \DIFdel{diffence }\DIFdelend \DIFaddbegin \DIFadd{difference }\DIFaddend between the two theories \autoref{sec:history} provides a bird's eye view of research spanning a period of nearly sixty years.

Those already familiar with the essentials of LQG (primarily area quantization) and the question of background independence of string theory can skip ahead to \autoref{sec:modified-ng} where we \DIFdelbegin \DIFdel{being }\DIFdelend \DIFaddbegin \DIFadd{begin }\DIFaddend by recalling the Nambu-Goto action and suggest a simple modification which incorporates our \textbf{central dogma}, which is that \emph{there must exist a smallest possible area $\Delta$ which is physically measurable.}\footnote{In recent work \cite{Borissova2024From} a very similar expression to ours for the modified Nambu-Goto action has been arrived at by considering a version of LQG called ``area metric gravity''.}
\DIFaddbegin 

\DIFadd{The key insight is to translate this fundamental discreteness of area from spacetime geometry to the worldsheet of the string itself. In LQG, the eigenvalues of the area operator are discrete, meaning physical areas can only take values from a quantized spectrum with minimum eigenvalue $\Delta$. For a string worldsheet, this suggests that the total worldsheet area $\mathcal{A} = \int d^2\sigma \sqrt{-h}$ cannot be arbitrarily small but must satisfy $\mathcal{A} \geq \Delta$.
}

\DIFadd{Rather than imposing this constraint externally, we incorporate it directly into the action by modifying the area element itself. The natural way to ensure the worldsheet area respects LQG's minimal area is to replace the standard area element $\sqrt{-h}$ with $\sqrt{-(h+k\Delta)}$ where $k = l_{pl}/l_s$ is the ratio of Planck to string scales. This modification has several appealing features: it reduces to the standard Nambu-Goto action when $k \to 0$, it ensures the worldsheet area can never become arbitrarily small, and it provides a concrete mechanism to incorporate LQG's geometric quantization into string theory.
}

\DIFadd{This approach differs from previous minimal length proposals in string theory \mbox{%DIFAUXCMD
\cite{Amati1989Canb,Chang2011Minimal,Gross1987Highenergy,Gross1988String,Yoneya1989Interpretation,Yoneya2000String} }\hskip0pt%DIFAUXCMD
in that it specifically targets area rather than length, reflecting LQG's emphasis on area as the fundamental geometric observable.
}

\DIFaddend We show that this action can be expanded in terms of a small coupling constant $k = l_{pl} / l_s$ where the Planck length - the length at which quantum geometric effects become important - $l_{pl}$ is much smaller than the string scale $l_s$ and therefore $k \ll 1$. We point out that the lowest order correction to the NG action can be interpreted in the context of bimetric gravity. In \autoref{sec:bimetric-string} we expand upon this observation and suggest a new action for the ``bimetric string'', along the lines of bimetric gravity. We find both, the Nambu-Goto and the \DIFdelbegin \DIFdel{Polaykov }\DIFdelend \DIFaddbegin \DIFadd{Polyakov }\DIFaddend forms of this action. Furthermore, we show that the phenomenological correction $\Delta$ introduced earlier is related to the pullback of the Kalb-Ramond field $B_{\mu \nu}$ into the worldsheet.
...
There are various appendices containing details of calculations and background information. In Appendix \ref{sec:modified-eom} we derive the equations of motion for the modified Nambu-Goto action. Appendix \ref{sec:polyakov-ng-relation} shows how the equations of motion extracted from the Nambu-Goto and \DIFdelbegin \DIFdel{Polakov }\DIFdelend \DIFaddbegin \DIFadd{Polyakov }\DIFaddend versions of the modified string action are equivalent to each other.
...
\end{equation}
While this procedure is straightforward in principle, in practice it was impossible to implement in the quantum theory due to the highly complicated non-polynomial dependence of the diffeomorphism and Hamiltonian constraints on the configuration and momentum variables. This unfortunate state of affairs persisted until the late 1980s when Abhay Ashtekar \cite{Ashketar1986New,Ashtekar1987New} recast general relativity as a BF theory of a dualised \DIFdelbegin \DIFdel{$\mathfrak{sl} (2 , \C)$ }\DIFdelend \DIFaddbegin \DIFadd{$\mathfrak{sl} (2, \C)$ }\DIFaddend connection (which becomes the self-dual part of said connection in the case of $\beta = i$)
...
\begin{subequations}
	...
	\end{equation}
\end{subequations}
(or equivalently $\diff x^{a} \rightarrow \diff x' _{a} = \Delta _{a b} \diff x^{b}$). This is a duality between the physics at the Planck scale and the physics at the string scale, \ie\DIFaddbegin \DIFadd{~ }\DIFaddend between strong coupling ($k' \gg 1$) and weak coupling ($k \ll 1$), small \DIFdelbegin \DIFdel{quantum of area ($\Delta$) and large quantum ofarea ($\Delta'$) and between }\DIFdelend \DIFaddbegin \DIFadd{worldsheet areas $h_{ab}$ and large worldsheet areas $h'_{ab}$ and between large string tension $T$ and small string tension $T'$.
}

\DIFadd{This duality is reminiscent of, but also distinct from, the T and S dualities in string theory. In T-duality the behavior of a string compactified on a circle of radius $R$ is related to the behavior of a string compactified on a circle of radius $\alpha'/R$. In S-duality the physics at strong string coupling $g_s \gg 1$ can be mapped to the physics at weak string coupling $g_s \ll 1$. In our case, the duality is between a description of physics at }\DIFaddend large string tension $T$ and small \DIFaddbegin \DIFadd{worldsheet areas $h_{ab}$ and a description of physics at small }\DIFaddend string tension $T'$ \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{and large worldsheet areas $h'_{ab} = (h_{ab})^{-1}$. This appears to have features of both T-duality and S-duality, in that the area inversion is reminiscent of T-duality and the string tension inversion is reminiscent of S-duality's strong-weak coupling correspondence. Physically, one way in which this duality could manifest in the real world is in the mass spectrum of string excitations which when computed in the strong-coupling regime should exhibit a precise correspondence with spectra in the weak-coupling limit under the transformation rules \eqref{eqn:T'}-\eqref{eqn:d2x}, while worldsheet correlation functions should satisfy similar duality relations, potentially providing calculable predictions for string phenomenology at both energy scales.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{As mentioned at the end of the previous section, the appeal of bimetric gravity lies in the fact that it eliminates the Boulware-Deser ghost in theories of massive gravity. However, what effect this would have on stringy dynamics, beyond the ansatz for a bimetric string action we have given above, and which to our knowledge is the first time such an expression has been written down, is beyond the scope of the present work and would require a separate paper in its own right.
}

\DIFaddend The second possibility is the following. Rather than considering two different sets of embedding fields, and consequently two different worldsheets, we can work with one set of embeddings $X^\mu(\tau,\sigma)$ and \emph{two different} bulk metrics $G_{\mu\nu}$ and $H_{\mu\nu}$. As a result, we would now get two different induced metrics on the \emph{same} string worldsheet, given by:
...
The expression \eqref{eqn:ws-bimetric-action-v2} is not ideal because it does not retain any memory of the form of the bulk metrics unless these are included in the interaction term $S'_{int}$. One way to manifestly exhibit the dependence of the respective worldsheet actions on the two bulk metrics is to work with the \DIFdelbegin \DIFdel{Polaykov }\DIFdelend \DIFaddbegin \DIFadd{Polyakov }\DIFaddend action.
...
\end{multline}
and completely \DIFdelbegin \DIFdel{analogous }\DIFdelend \DIFaddbegin \DIFadd{analogously }\DIFaddend for the $H_{\mu \nu}$ metric,
...
\begin{equation} \DIFdelbegin %DIFDELCMD < \label{eqn:h=SgS}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \label{eqn:h-delta-relation}
	\DIFaddend h_{a b} = \Delta _{a c} \Delta _{b d} g^{c d}.
\end{equation}
\DIFaddbegin \DIFadd{Taking the determinant of both sides of the above equation gives us $h = \Delta ^2 / g$.
}\DIFaddend By writing the inverse metric $g^{a b}$ in terms of Levi-Civita symbols $g^{a b} = \varepsilon ^{a c} \varepsilon ^{b d} g_{c d} / 2 g$\DIFaddbegin \footnote{\DIFadd{This expression for the inverse of a metric is probably not familiar to many people, but its correctness can be checked by contracting both sides of the expression with $g_{ab}$ and seeing that the resulting expression is the much better known expression for the determinant $g \equiv \det{g_{ab}}$}}\DIFaddend , this implies that
...
\begin{equation} \DIFdelbegin %DIFDELCMD < \label{eqn:S=sigma_y}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \label{eqn:delta-epsilon-relation}
	\DIFaddend \Delta _{a c} = \sqrt{\Delta} \varepsilon _{a c}
\end{equation}
...
\begin{equation} \DIFdelbegin %DIFDELCMD < \label{eqn:H=1/(2g)G}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \label{eqn:H-G-relation}
...
%DIFDELCMD < %%%
\DIFdelend If we impose the conditions \DIFdelbegin \DIFdel{\eqref{eqn:h=SgS}, \eqref{eqn:S=sigma_y} and \eqref{eqn:H=1/(2g)G} }\DIFdelend \DIFaddbegin \DIFadd{\eqref{eqn:h-delta-relation}, \eqref{eqn:delta-epsilon-relation} and \eqref{eqn:H-G-relation} }\DIFaddend plus $T' = - T k / 2$ in the interactionless bimetric Polyakov action \eqref{eqn:polyakov-bimetric}, we obtain the Polyakov version of the area-corrected action \eqref{eqn:modified-ng-action}:
...
\end{equation}
thus the action \eqref{eqn:ng-area-corrected} can be recast as a Born-Infeld model for the string with $k$ playing the role of inverse tension squared and $\Delta _{a b}$ the role of the pullback of a field strength. \DIFdelbegin \DIFdel{Since we explored the relation between $\Delta _{a b}$ and the Kalb-Ramond field $B_{\mu \nu}$ in }%DIFDELCMD < \autoref{sec:WS-Kalb-Ramond-QoA}%%%
\DIFdel{, this could mean that the Kalb-Ramond 2-form is exact, $B = \diff A$, and thus its exterior derivative, which gives the torsion of the background spacetime \mbox{%DIFAUXCMD
\cite{Tong2009Lectures}}\hskip0pt%DIFAUXCMD
, vanishes $\diff B = \diff (\diff A) = 0$. }\DIFdelend Also, we have a direct relation between string tension and scale
...
%DIFDELCMD < %%%
\DIFdelend When looking at the constraints, we see that we can't decouple the exponentials in $\tau$ from any of the sums, thus we get two-indices \textquotedblleft Virasoro modes"
\begin{subequations}
	\begin{align}
		L\DIFdelbegin \DIFdel{_{n , p} }\DIFdelend \DIFaddbegin \DIFadd{_{n, p} }\DIFaddend := \alpha ^{\mu} _{n} \alpha ^{\nu} _{p} \eta _{\mu \nu}                                                         & = 0, \ \forall \ n, p \in \mathbb{Z}, \label{eqn:"virasoro"-modes-1} \\
		\widetilde{L}\DIFdelbegin \DIFdel{_{n , p} }\DIFdelend \DIFaddbegin \DIFadd{_{n, p} }\DIFaddend := \alpha ^{\mu} _{n} (\alpha ^{\nu} _{p})^{*} \eta _{\mu \nu}                                       & = 0, \ n \neq p, \label{eqn:"virasoro"-modes-2}                       \\
		\sum _{n} \left( \left( 1 + \frac{n}{E_{n}} \right) ^2 \alpha ^{\mu} _{n} (\alpha ^{\nu} _{n})^{*} \eta _{\mu \nu} \right) & = 0 \label{eqn:"virasoro"-modes-sum} .
	\end{align}
\end{subequations}
Constraint \eqref{eqn:"virasoro"-modes-2} implies that we only have $D$ independent expansion modes for each field $X^{\mu}$, which for consistency as we will see later means \DIFdelbegin \DIFdel{$\alpha ^{\mu} _{- p} = \alpha ^{\mu} _{p} , \ p \leq d = D - 1$ and $\alpha ^{\mu} _{n} = 0 , \ \forall \ |n| > d$}\DIFdelend \DIFaddbegin \DIFadd{$\alpha ^{\mu} _{- p} = \alpha ^{\mu} _{p}, \ p \leq d = D - 1$ and $\alpha ^{\mu} _{n} = 0, \ \forall \ |n| > d$}\DIFaddend , so we have a truncated solution. This is not surprising, since because space-time is discrete we expect wavelengths smaller than the granular structure to be ruled out.
...
\begin{multline} \label{eqn:string-rest-mass-classical-1}
	M^2 = - p^{\mu} p_{\mu} = \frac{4}{\alpha '} \Bigg( \sum _{n \neq 0} \left( \left( 1 + \frac{n}{E_{n}} \right) ^2 \widetilde{L}\DIFdelbegin \DIFdel{_{n , n} }\DIFdelend \DIFaddbegin \DIFadd{_{n, n} }\DIFaddend \right) - \\
...
\begin{subequations}
	\begin{align}
		...
		\left\{ \alpha ^{\mu} _{n}, (\alpha ^{\nu} _{p})^{*} \right\} & = - i E_{n} \delta \DIFdelbegin \DIFdel{_{|n| , |p|} }\DIFdelend \DIFaddbegin \DIFadd{_{|n|, |p|} }\DIFaddend \eta ^{\mu \nu}, \label{eqn:expansion-modes-bracket-1}
	\end{align}
\end{subequations}
...
\begin{equation} \label{eqn:harmonic-bracket-relations}
	\left\{ a^{\mu} _{n}, (a^{\nu} _{p})^{*} \right\} = - i \delta \DIFdelbegin \DIFdel{_{|n| , |p|} }\DIFdelend \DIFaddbegin \DIFadd{_{|n|, |p|} }\DIFaddend \eta ^{\mu \nu} .
\end{equation}
...
\begin{subequations}
	\begin{equation} \label{eqn:L-brackets-0}
		\left\{ L\DIFdelbegin \DIFdel{_{n , m} }\DIFdelend \DIFaddbegin \DIFadd{_{n, m}}\DIFaddend , L\DIFdelbegin \DIFdel{_{k , p} }\DIFdelend \DIFaddbegin \DIFadd{_{k, p} }\DIFaddend \right\} = 0
	\end{equation}
	\begin{equation} \label{eqn:L-brackets-1}
		\left\{ L\DIFdelbegin \DIFdel{_{n , m} }\DIFdelend \DIFaddbegin \DIFadd{_{n, m}}\DIFaddend , \widetilde{L}\DIFdelbegin \DIFdel{_{k , p} }\DIFdelend \DIFaddbegin \DIFadd{_{k, p} }\DIFaddend \right\} = - i \left( L\DIFdelbegin \DIFdel{_{m , k} }\DIFdelend \DIFaddbegin \DIFadd{_{m, k} }\DIFaddend \delta \DIFdelbegin \DIFdel{_{|n| , |p|} }\DIFdelend \DIFaddbegin \DIFadd{_{|n|, |p|} }\DIFaddend + L\DIFdelbegin \DIFdel{_{n , k} }\DIFdelend \DIFaddbegin \DIFadd{_{n, k} }\DIFaddend \delta \DIFdelbegin \DIFdel{_{|m| , |p|} }\DIFdelend \DIFaddbegin \DIFadd{_{|m|, |p|} }\DIFaddend \right)
	\end{equation}
	\begin{equation} \label{eqn:L-brackets-2}
		\left\{ \widetilde{L}\DIFdelbegin \DIFdel{_{n , m} }\DIFdelend \DIFaddbegin \DIFadd{_{n, m}}\DIFaddend , \widetilde{L}\DIFdelbegin \DIFdel{_{k , p} }\DIFdelend \DIFaddbegin \DIFadd{_{k, p} }\DIFaddend \right\} = - i \left( \widetilde{L}\DIFdelbegin \DIFdel{_{k , m} }\DIFdelend \DIFaddbegin \DIFadd{_{k, m} }\DIFaddend \delta \DIFdelbegin \DIFdel{_{|n| , |p|} }\DIFdelend \DIFaddbegin \DIFadd{_{|n|, |p|} }\DIFaddend - \widetilde{L}\DIFdelbegin \DIFdel{_{n , p} }\DIFdelend \DIFaddbegin \DIFadd{_{n, p} }\DIFaddend \delta \DIFdelbegin \DIFdel{_{|m| , |k|} }\DIFdelend \DIFaddbegin \DIFadd{_{|m|, |k|} }\DIFaddend \right) .
	\end{equation}
\end{subequations}

\section{The Quantum Klein-Gordon String} \label{sec:quantum-kg-string}

We now seek to quantize the Klein-Gordon string by means of covariant quantization, in a manner much similar to those found in \cite{Tong2009Lectures,Green2012Superstring}. The string coordinates $X^{\mu}$ and it's conjugate momentum $\Pi ^{\nu}$ are turned into operators $\Hat{X}^{\mu}$ and $\Hat{\Pi}^{\nu}$, with the Poisson brackets \DIFdelbegin \DIFdel{$\{ . , . \}$ }\DIFdelend \DIFaddbegin \DIFadd{$\{ ., . \}$ }\DIFaddend being replaced by commutators \DIFdelbegin \DIFdel{$\{ . , . \} \rightarrow - i [ . , . ]$}\DIFdelend \DIFaddbegin \DIFadd{$\{ ., . \} \rightarrow - i [ ., . ]$}\DIFaddend :
\begin{subequations}
	\begin{align}
		...
		\left[ \Hat{a}^{\mu} _{n}, (\Hat{a}^{\nu} _{p})^{\dagger} \right]               & = \delta \DIFdelbegin \DIFdel{_{|n| , |p|} }\DIFdelend \DIFaddbegin \DIFadd{_{|n|, |p|} }\DIFaddend \eta ^{\mu \nu} \label{eqn:a-a*-commutator}                                                      \\
...
%DIFDELCMD < %%%
\DIFdelend As usual, the appearance of the Minkowski metric in the non-zero commutator
\begin{equation} \label{eqn:ghosts}
	\Big[ \Hat{a}^{\mu} _{p}, (\Hat{a}^{\nu} _{k})^{\dagger} \Big] = \eta ^{\mu \nu} \delta \DIFdelbegin \DIFdel{_{|p| , |k|} }\DIFdelend \DIFaddbegin \DIFadd{_{|p|, |k|}}\DIFaddend ,
\end{equation}
...
\begin{subequations}
	\begin{equation} \label{eqn:L-constraint-quantum-1}
		\bra{\phi} \Hat{L}\DIFdelbegin \DIFdel{_{p , k} }\DIFdelend \DIFaddbegin \DIFadd{_{p, k} }\DIFaddend \ket{\psi} = 0 \, \,
	\end{equation}
	\begin{equation} \label{eqn:L-constraint-quantum-2}
		\bra{\phi} \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{p , k} }\DIFdelend \DIFaddbegin \DIFadd{_{p, k} }\DIFaddend \ket{\psi} = 0 .
	\end{equation}
\end{subequations}
\DIFdelbegin \DIFdel{$\Hat{L}_{p , k}$ }\DIFdelend \DIFaddbegin \DIFadd{$\Hat{L}_{p, k}$ }\DIFaddend has no ambiguities since it's composed of only annihilation operators. As for \DIFdelbegin \DIFdel{$\widetilde{L}_{p , k}$}\DIFdelend \DIFaddbegin \DIFadd{$\widetilde{L}_{p, k}$}\DIFaddend , we have an ambiguity for $p = k \neq 0$, so we pick normal ordering with the annihilation operator to the right
\begin{equation} \label{eqn:L-normal-ordering}
	\Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{p , p} }\DIFdelend \DIFaddbegin \DIFadd{_{p, p} }\DIFaddend = (\Hat{\alpha}^{\mu} _{p})^{\dagger} \Hat{\alpha}^{\nu} _{p} \eta _{\mu \nu} .
\end{equation}
This ambiguity manifests in the imposition of the constraint as
\begin{equation} \label{eqn:shifted-L-constraint}
	\bra{\phi} \left( \sum _{n \neq 0} \left( \left( 1 + \frac{n}{E_{n}} \right) ^2 \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{n , n} }\DIFdelend \DIFaddbegin \DIFadd{_{n, n} }\DIFaddend \right) - a \right) \ket{\psi} = 0,
\end{equation}
...
\end{multline}
This constant can be calculated by imposing normal-ordering on the summed constraint since \DIFdelbegin \DIFdel{$n \in (- d , d)$ }\DIFdelend \DIFaddbegin \DIFadd{$n \in (- d, d)$ }\DIFaddend is finite, and the result is
\begin{multline}
...
\DIFaddbegin \DIFadd{This table shows that $a$ becomes more negative as either the spacetime dimension $D$ or the mass parameter $\mu$ increases. This means:
}

\begin{itemize}
	\item \DIFadd{Higher dimensions lead to more massive ground states
	}\item \DIFadd{Larger $\mu$ (stronger discreteness effects) also increases ground state masses
	}\item \DIFadd{The spectrum becomes increasingly separated from the massless limit
}\end{itemize}

\DIFadd{For example, in $D=2$ with $\mu=1$, $a=-4.24$ gives a relatively light spectrum, while $D=3$ with $\mu=5$ yields $a=-34.27$, producing much heavier states. This demonstrates how LQG's discreteness parameter $\mu$ and the background dimensionality jointly control the departure from standard string theory's massless spectrum, with stronger quantum geometric effects pushing all states away from the massless limit expected in conventional string theory.
}

\DIFaddend The commutation relations between the constraints are inherited by the Poisson brackets
\begin{subequations}
	\begin{align}
		\left[ \Hat{L}\DIFdelbegin \DIFdel{_{n , m} }\DIFdelend \DIFaddbegin \DIFadd{_{n, m}}\DIFaddend , \Hat{L}\DIFdelbegin \DIFdel{_{k , p} }\DIFdelend \DIFaddbegin \DIFadd{_{k, p} }\DIFaddend \right]                         & = 0 \label{eqn:L-commutator-0}                                                                                                   \\
		\left[ \Hat{L}\DIFdelbegin \DIFdel{_{n , m} }\DIFdelend \DIFaddbegin \DIFadd{_{n, m}}\DIFaddend , \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{k , p} }\DIFdelend \DIFaddbegin \DIFadd{_{k, p} }\DIFaddend \right]             & = \Hat{L}\DIFdelbegin \DIFdel{_{m , k} }\DIFdelend \DIFaddbegin \DIFadd{_{m, k} }\DIFaddend \delta \DIFdelbegin \DIFdel{_{|n| , |p|} }\DIFdelend \DIFaddbegin \DIFadd{_{|n|, |p|} }\DIFaddend + \Hat{L}\DIFdelbegin \DIFdel{_{n , k} }\DIFdelend \DIFaddbegin \DIFadd{_{n, k} }\DIFaddend \delta \DIFdelbegin \DIFdel{_{|m| , |p|} }\DIFdelend \DIFaddbegin \DIFadd{_{|m|, |p|} }\DIFaddend \label{eqn:L-commutator-1}                           \\
		\left[ \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{n , m} }\DIFdelend \DIFaddbegin \DIFadd{_{n, m}}\DIFaddend , \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{k , p} }\DIFdelend \DIFaddbegin \DIFadd{_{k, p} }\DIFaddend \right] & = \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{k , m} }\DIFdelend \DIFaddbegin \DIFadd{_{k, m} }\DIFaddend \delta \DIFdelbegin \DIFdel{_{|n| , |p|} }\DIFdelend \DIFaddbegin \DIFadd{_{|n|, |p|} }\DIFaddend - \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{n , p} }\DIFdelend \DIFaddbegin \DIFadd{_{n, p} }\DIFaddend \delta \DIFdelbegin \DIFdel{_{|m| , |k|} }\DIFdelend \DIFaddbegin \DIFadd{_{|m|, |k|} }\DIFaddend \label{eqn:L-commutator-2} .
	\end{align}
\end{subequations}
...
\begin{multline} \label{eqn:L-commutator-anomalous-initial}
	\left[ \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{n , m} }\DIFdelend \DIFaddbegin \DIFadd{_{n, m}}\DIFaddend , \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{k , p} }\DIFdelend \DIFaddbegin \DIFadd{_{k, p} }\DIFaddend \right] = \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{k , m} }\DIFdelend \DIFaddbegin \DIFadd{_{k, m} }\DIFaddend \delta \DIFdelbegin \DIFdel{_{|n| , |p|} }\DIFdelend \DIFaddbegin \DIFadd{_{|n|, |p|} }\DIFaddend - \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{n , p} }\DIFdelend \DIFaddbegin \DIFadd{_{n, p} }\DIFaddend \delta \DIFdelbegin \DIFdel{_{|m| , |k|} }\DIFdelend \DIFaddbegin \DIFadd{_{|m|, |k|} }\DIFaddend + \\
	+ C_{n} \delta \DIFdelbegin \DIFdel{_{|n| , |p|} }\DIFdelend \DIFaddbegin \DIFadd{_{|n|, |p|} }\DIFaddend + D_{k} \delta \DIFdelbegin \DIFdel{_{|m| , |k|} }\DIFdelend \DIFaddbegin \DIFadd{_{|m|, |k|} }\DIFaddend .
\end{multline}
...
\begin{subequations}
	\begin{align}
		\Hat{H} & = - \frac{1}{2} \sum _{n} \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{n , n} }\DIFdelend \DIFaddbegin \DIFadd{_{n, n} }\DIFaddend \label{eqn:WS-Hamiltonian}              \\
		\Hat{P} & = - \frac{1}{2} \sum _{n} \frac{n}{E_{n}} \Hat{\widetilde{L}}\DIFdelbegin \DIFdel{_{n , n} }\DIFdelend \DIFaddbegin \DIFadd{_{n, n} }\DIFaddend \label{eqn:WS-momentum}
	\end{align}
\end{subequations}
...
\end{multline}
while the auxiliary \DIFdelbegin \DIFdel{$\bra{0 ; p} \Hat{\widetilde{L}}_{n , 0} \zeta _{\mu '} (\Hat{a}^{\mu '} _{d})^{\dagger} \ket{0 ; p} = 0$ }\DIFdelend \DIFaddbegin \DIFadd{$\bra{0 ; p} \Hat{\widetilde{L}}_{n, 0} \zeta _{\mu '} (\Hat{a}^{\mu '} _{d})^{\dagger} \ket{0 ; p} = 0$ }\DIFaddend constraint implies that
...
\end{equation}
The auxiliary \DIFdelbegin \DIFdel{$\bra{0 ; p} \xi _{\alpha} \Hat{a}^{\alpha} _{n} \Hat{\widetilde{L}}_{n , 0} (\Hat{a}^{\mu} _{n})^{\dagger} (\Hat{a}^{\nu} _{n})^{\dagger} \zeta _{\mu \nu} \ket{0 ; p} = 0$ }\DIFdelend \DIFaddbegin \DIFadd{$\bra{0 ; p} \xi _{\alpha} \Hat{a}^{\alpha} _{n} \Hat{\widetilde{L}}_{n, 0} (\Hat{a}^{\mu} _{n})^{\dagger} (\Hat{a}^{\nu} _{n})^{\dagger} \zeta _{\mu \nu} \ket{0 ; p} = 0$ }\DIFaddend condition where $\xi _{\alpha}$ is a arbitrary polarization for the auxiliary vector excitation implies that
...
\begin{subequations}
	...
	\end{align}
\end{subequations}
$\zeta _{\mu \nu} = G_{\mu \nu} + B_{\mu \nu} + \Phi \eta _{\mu \nu}$, giving rise to 3 well-known types of background fields: the spacetime metric $G_{\mu \nu}$, Kalb-Ramond 2-form $B_{\mu \nu}$ and dilaton $\Phi$, all of those having squared mass given by eq \eqref{eqn:nth-spin-2-mass-shell}. We thus see that the symmetric part $S_{\mu \nu} = G_{\mu \nu} + \Phi \eta _{\mu \nu}$ of $\zeta _{\mu \nu}$ is orthogonal to the momentum of the string, $S_{\mu \nu} p^{\mu} = 0$, and since $p^2 < 0$, we can choose a reference frame where \DIFdelbegin \DIFdel{$p^{\mu} = (p , 0 , ... , 0)$ }\DIFdelend \DIFaddbegin \DIFadd{$p^{\mu} = (p, 0, ..., 0)$ }\DIFaddend only has a time component, leaving us with $S_{0 \nu} = 0$, implying that $\zeta _{0 \nu} = - \zeta _{\nu 0}$. This condition guarantees that the timelike components of $\zeta _{\mu \nu}$ have a positive contribution to its square \DIFdelbegin \DIFdel{$\zeta ^{\mu \nu} \zeta _{\mu \nu} = \zeta ^{i j} \zeta _{i j} + b , \ b = \zeta ^{0 i} \zeta _{0 i} > 0$}\DIFdelend \DIFaddbegin \DIFadd{$\zeta ^{\mu \nu} \zeta _{\mu \nu} = \zeta ^{i j} \zeta _{i j} + b, \ b = \zeta ^{0 i} \zeta _{0 i} > 0$}\DIFaddend , and the remaining part is spacelike thus must also have positive square, so we get that $\zeta ^{\mu \nu} \zeta _{\mu \nu} > 0$, so we are free of ghosts in the spin-2 spectrum.
...
\begin{equation} \label{eqn:G-polarizations}
	\frac{1}{2} D (D - 1) - 1
\end{equation}
possible polarizations, which shows that indeed those polarizations fit nicely in a representation of \DIFdelbegin \DIFdel{$\mathrm{SO} (D - 1 , 1)$, \ie}\DIFdelend \DIFaddbegin \DIFadd{$\mathrm{SO} (D - 1, 1)$, \ie~ }\DIFaddend the traceless symmetric tensor representation, thus all our excited spin-2 states are consistent with Lorentz symmetry at the macroscopic scale even tho near the Planck scale they deviate a bit by having an effective mass.

Although our method is quite different from the one in \cite{Thiemann2004The-LQG-String:}, we \DIFdelbegin \DIFdel{got }\DIFdelend \DIFaddbegin \DIFadd{get }\DIFaddend similar results: a quantum Virasoro-like algebra with no anomalies, unconstrained background dimensionality $D$ and no negative-norm states in physical processes.

\DIFaddbegin \DIFadd{It is important to clarify what we mean by ``no anomalies'' in this context. This refers specifically to the absence of central charge anomalies in our modified Virasoro-like algebra, which arises due to the finite mode truncation imposed by LQG's discreteness. This discreteness is encoded in the paramter $\mu$ in \eqref{eqn:mu-constraint-1}, which is a cutoff parameter that controls the maximum number of modes we consider in the action.
}

\DIFadd{This differs fundamentally from standard string theory, where anomaly cancellation requires critical dimensions ($D = 26$ for bosonic strings) to ensure vanishing of the central charge from an infinite sum over modes. Our finite sum naturally avoids this constraint, permitting unconstrained background dimensionality $D$. However, this result applies specifically to our truncated model in the regime where $k \ll 1$, where the smooth worldsheet approximation remains valid. In the full quantum gravitational regime where $k \sim 1$, the discrete structure of the underlying spin network becomes manifest and a more comprehensive analysis would be required.
}

\DIFadd{Additionally, our anomaly-freedom and ghost-freedom analysis is limited to the tree-level constraints; quantum loop corrections and the full quantum effective action may introduce subtleties not captured in this treatment.
}

\DIFadd{The mode truncation is a key assumption that requires validation. While we do not have a rigorous derivation from first principles, the physical motivation comes from LQG's discrete structure: high-frequency modes corresponding to sub-Planckian physics should be suppressed when the spin network discreteness becomes relevant. The parameter $\mu$ in \eqref{eqn:mu-constraint-1} provides an effective energy cutoff, with modes $|n| > d \approx \mu$ being suppressed.
}

\DIFadd{This approach shares similarities with other discrete quantum gravity approaches like causal dynamical triangulation and spin foam models, which also impose natural cutoffs. However, a complete justification would require deriving this truncation directly from LQG's canonical quantization procedure, which remains an open question for future work.
}

\DIFaddend \section{Emergent Dimensions and LQG} \label{sec:emergent-dimensions}
...
\end{equation}
where $E_{\alpha} ^2$ does not mean $E_{\alpha} E^{\alpha}$, rather it means \DIFdelbegin \DIFdel{$(E_{3})^2 , (E_{4})^2 , ... , (E_{D - 1})^2$, \ie}\DIFdelend \DIFaddbegin \DIFadd{$(E_{3})^2, (E_{4})^2, ..., (E_{D - 1})^2$, \ie~}\DIFaddend , the squared WS energy (and momentum) of each field $X^{\mu}$.

Afshordi's work notes that the action \eqref{eqn:emergent-dimensions-action} only has background Poincaré invariance and WS Weyl symmetry as approximate symmetries for the fields with $m_{\mu} \gg T$, thus the \textquotedblleft emergent spacetime and evolving dimensions" by interpreting the fields with effective mass as being inaccessible due to not obeying Poincaré symmetry. We go one step further and note that since eq \eqref{eqn:X-masses-constraint} leaves \DIFdelbegin \DIFdel{$m_{0} , m_{1}$ }\DIFdelend \DIFaddbegin \DIFadd{$m_{0}, m_{1}$ }\DIFaddend and $m_{2}$ unconstrained, the related dimensions might be regarded as being \emph{fundamental}\DIFaddbegin \footnote{\DIFadd{Of course, the notion of what is ``fundamental'' is relative. After all, this is a phenomenological model and a very crude one at that. Moreover if holography is correct then it stands to reason that our $3+1$ dimensional bulk geometry has an equivalent description in terms of a theory living on a $2 + 1$ dimensional  ``boundary''. And if one takes the argument to its logical limit then there is no reason }\emph{\DIFadd{not}} \DIFadd{to reduce further from $2+1$ $\rightarrow$ $1+1$ $\rightarrow$ $0+1$ dimensions. In fact, this is what proponents of matrix models would argue that }\emph{\DIFadd{is}} \DIFadd{the ``correct'' picture.}} \DIFaddend and thus the true quantum gravitational field should be $(2 + 1)$-dimensional, in agreement with \cite{t-Hooft1993Dimensional}. $(2 + 1)$-dimensional gravity has already been successfully quantized before (\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Bergshoeff2013ZweiDreibein}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Bergshoeff2013Zwei-Dreibein}}\hskip0pt%DIFAUXCMD
}\DIFaddend ,\cite{Blais2013A-new-quantization},\cite{Carlip2003Quantum}). We thus postulate that $m_{3} \sim T_{pl}$, which together with the constraints \eqref{eqn:X-masses-constraint} should give us a way to calculate the other mass parameters given the WS energy and momentum of the fields, making it possible to predict in which scales the other dimensions should start to open up.

\DIFaddbegin \DIFadd{Our constraint equation (\ref{eqn:X-masses-constraint}) represents an attempt to bridge Afshordi's phenomenological emergent dimension model with LQG's general prediction of modified physics at the Planck scale. However, the specific form of these constraints and their derivation from first-principles LQG remains an open problem. While both frameworks involve discreteness - Afshordi's through hierarchical mass parameters and LQG through quantized geometric operators - establishing a concrete mathematical connection requires significantly more work.
}

\DIFadd{This represents an important limitation of our current approach and highlights the need for future research to either derive such constraints from canonical LQG or develop alternative frameworks that more naturally incorporate both emergent dimensions and quantum geometric discreteness. Similar caveats also apply to our suggestion that \eqref{eqn:X-masses-constraint} should imply that spacetime is $(2 + 1)$-dimensional.
}

\DIFaddend By analysing the equations of motion for the metric $g_{c d}$ from the action \eqref{eqn:emergent-dimensions-action} we get
...
\begin{equation}\DIFaddbegin \label{eqn:enlarged-Xs-transform}
	\DIFaddend \begin{pmatrix}
		X^\mu \\
		1
...
\begin{equation}\label{eqn:cosmological-constant}
	\DIFdelbegin \DIFdel{\lambda }\DIFdelend \DIFaddbegin \DIFadd{\Lambda }\DIFaddend = \frac{3 \ep}{l^2}.
\end{equation}
\DIFaddbegin \DIFadd{Given the observed cosmological constant $\Lambda \approx 1.47 \times 10^{-52} \text{m}^{-2}$ \mbox{%DIFAUXCMD
\cite{Collaboration2020Planck}}\hskip0pt%DIFAUXCMD
, eq \eqref{eqn:cosmological-constant} implies that $l \approx 1.4 \times 10^{26} \text{m}$. This happens to coincide with the scale of the cosmological horizon \mbox{%DIFAUXCMD
\cite{Peebles2003Cosmological}}\hskip0pt%DIFAUXCMD
. This would suggest that the scale of the emergent dimension $X^r$ is of the order of the cosmological horizon. Of course, this is a very naive ``back of the envelope'' calculation and would require further investigation to place on a firmer footing.
}

\DIFaddend One might ask what is the motivation for introducing dS (adS) geometries into the discussion. The main motivation of course is the Maldacena or adS/CFT correspondence. The adS spacetime is usually understood to arise as a consequence of non-perturbative effects which nobody really has a proper understanding of as yet. One might hope that for a correspondence which has passed all possible checks uptil now, there should be some way to realise it in the classical theory also.
...
\DIFaddbegin \DIFadd{The key observation which enables this is that $X^r$ behaves as a scaling or ``holographic'' dimension as explained at the end of }\autoref{sec:translation_gauge}\DIFadd{.
}

\DIFadd{Our classical construction provides a geometric framework that naturally incorporates key features of the AdS/CFT correspondence. The scaling dimension $X^r$ introduced through the mathematical necessity of closing the Poincaré algebra plays a role analogous to the radial coordinate in AdS space. Just as in AdS/CFT where the radial direction encodes the energy scale of the boundary theory, variations in our $X^r$ coordinate directly control the scale transformations of the bulk coordinates $X^\mu$. This creates a natural holographic setup where information about different energy scales in the ``boundary'' ($X^r = \text{constant}$ surfaces) is encoded in the ``bulk'' (the extended geometry with the extra $X^r$ dimension).
}

\DIFadd{However, our construction differs from standard AdS/CFT in several important ways: (1) it emerges at the classical level from gauge theory requirements rather than from quantum/non-perturbative effects, (2) the holographic direction arises from the string action itself rather than being an external bulk geometry, and (3) the correspondence is between worldsheet dynamics and the extended embedding space rather than between a boundary CFT and bulk gravity. While this provides a concrete classical realization of holographic principles, establishing a precise correspondence with Maldacena's duality would require demonstrating how our construction reproduces the specific correlation functions and scaling behaviors characteristic of AdS/CFT, which remains an open challenge for future work.
}

\DIFaddend \subsection{Gauged Polyakov Equations of Motion}
...
\end{equation}
from which the pulledback connection \DIFdelbegin \DIFdel{$\mc{A}_{a} = \del _{a} X^{\mu} \mc{A}_{\mu}$ is thus
}\begin{displaymath} \DIFdel{%DIFDELCMD < \label{eqn:adS-WS-connection}%%%
	\mc{A}_{a} = \begin{bmatrix} 0 & 0 & 0 & 0 & \del _{a} X^{0} \\ 0 & 0 & 0 & 0 & \del _{a} X^{1} \\ 0 & 0 & 0 & 0 & \del _{a} X^{2} \\ 0 & 0 & 0 & 0 & \del _{a} X^{3} \\ - \del _{a} X^{0} & \del _{a} X^{1} & \del _{a} X^{2} & \del _{a} X^{3} & 0 \end{bmatrix} ,
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{is $\mc{A}_{a} = \del _{a} X^{\mu} \mc{A}_{\mu}$, }\DIFaddend whereby imposing conformal gauge on the induced metric $\gamma _{a b} = \del _{a} X^{\mu} \del _{b} X^{\nu} \eta _{\mu \nu} = \e ^{\varphi} \eta _{a b}$ we can separate equations for the Minkowski sector $X^{\mu}$ and for the holographic dimension $X^{r}$:
...
\begin{equation} \label{eqn:simplified-holographic-EoM}
	\eta ^{a b} \del _{a} \del _{b} X^{r} + 2 \mu ^2 \e ^{\varphi} X^{r} = - 4 \mu \e ^{\varphi} .
\end{equation}
Thus, the Minkowski sector behaves like regular ST for small perturbations, \DIFdelbegin \DIFdel{and for the holographic dimension we can solve using a Green's function
}\begin{displaymath} \DIFdel{%DIFDELCMD < \label{eqn:Xr-GF}%%%
	X^{r} (x) = - 4 \mu \int \diff ^2 x' G (x ; x') \e ^{\varphi (x')} ,
}\end{displaymath}%DIFAUXCMD
\DIFdel{with the Green's function satisfying
}\begin{displaymath} \DIFdel{%DIFDELCMD < \label{eqn:Greens-function}%%%
	(\del _{\sigma} ^2 - \del _{\tau} ^2 + 2 \mu ^2 \e ^{\varphi}) G (x ; x') = \delta ^2 (x , x') .
}\end{displaymath}%DIFAUXCMD
\DIFdel{To find this Green's function, we must fix the conformal factor to be of the form $\e ^{\varphi} = f (\tau) - g (\sigma)$, thus allowing us to separate variables in the homogeneous case of eq \eqref{eqn:Greens-function} into
}\begin{displaymath} \DIFdel{%DIFDELCMD < \label{eqn:Green-X}%%%
	X^{\prime \prime} - 2 \mu ^2 g (\sigma) X = - \lambda X
}\end{displaymath}%DIFAUXCMD
\begin{displaymath} \DIFdel{%DIFDELCMD < \label{eqn:Green-T}%%%
	\ddot{T} - 2 \mu ^2 f (\tau) T = - \lambda T .
}\end{displaymath}%DIFAUXCMD
\DIFdel{These are Schrodinger's equations for potentials given by $\mu f (\tau)$ and $\mu g (\sigma)$, as such if we assume that $f$ and $g$ are harmonic oscillator potentials $f (\tau) = \omega ^2 \tau ^2 / 2$ and $g (\sigma) = k^2 \sigma ^2 / 2$ we can find known solutions for separation constant $\lambda = 2 \mu E = \mu k (2n + 1) = \mu \omega (2n + 1)$, so by the Spectral Theorem our separated spatial and temporal }\DIFdelend \DIFaddbegin \DIFadd{while the holographic coordinate $X^{r}$ follows a variable-mass Klein-Gordon equation with source. Now it should be clear why the choice of $\mu$ for the coupling constant, since in \eqref{eqn:simplified-holographic-EoM} it appears (together with the conformal factor) as a mass term for our perturbation. Eq \eqref{eqn:simplified-holographic-EoM} can be solved using the 1d wave equation }\DIFaddend Green's \DIFdelbegin \DIFdel{functions are
}\begin{displaymath} \DIFdel{%DIFDELCMD < \label{eqn:X-function}%%%
	X (\sigma , \sigma ') = \sum _{n \geq 0} - \frac{X_{n} (\sigma) X_{n} (\sigma ')}{\mu k (2 n + 1)}
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{function to build a perturbation around the homogenous wave equation solution
}\begin{multline} \DIFadd{\label{eqn:holo-sol-implicit}
	X^{r} (x) = X^{r} _{0} (x) - }\\ \DIFadd{- 2 \mu \int \diff ^2 x' G (x ; x') \e ^{\varphi (x')} (2 + \mu X^{r} (x')) = }\\ \DIFadd{= X^{r} _{0} (x) + \delta X^{r} (x),
}\end{multline}\DIFaddend 
\DIFdelbegin \begin{displaymath} \DIFdel{%DIFDELCMD < \label{eqn:T-function}%%%
	T (\tau , \tau ') = \sum _{m \geq 0} - \frac{T_{m} (\tau) T_{m} (\tau ')}{\mu \omega (2 m + 1)} ,
}\end{displaymath}%DIFAUXCMD
\DIFdel{where $X_{n}$ }\DIFdelend \DIFaddbegin \DIFadd{where $X^{r} _{0}$ is a solution for the homogenous wave equation
}\begin{equation} \DIFadd{\label{eqn:homo-wave-eqn}
	(\partial _{\sigma} ^{2} - \del _{\tau} ^{2}) X^{r} _{0} = 0
}\end{equation}
\DIFaddend and \DIFdelbegin \DIFdel{$T_{m}$ are harmonic oscillator eigenfunctions. With this, the full }\DIFdelend \DIFaddbegin \DIFadd{$G (x ; x') = \frac{1}{2} \Theta (\tau - \tau ' - |\sigma - \sigma '|)$ its }\DIFaddend Green's function
\DIFdelbegin \DIFdel{$G = T.X$ and the solution for $X^{r}$ are thus
}\DIFdelend \begin{equation} \DIFdelbegin %DIFDELCMD < \label{eqn:G-function}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \label{eqn:homo-wave-eqn-GF}
	\DIFadd{(\partial _{\sigma} ^{2} - \del _{\tau} ^{2}) }\DIFaddend G (x ; x') = \DIFdelbegin \DIFdel{\sum _{m \geq 0} \sum _{n \geq 0} \frac{T_{m} (\tau) T_{m} (\tau ')}{\mu \omega (2 m + 1)} \frac{X_{n} (\sigma) X_{n} (\sigma ')}{\mu k (2 n + 1)}
}\DIFdelend \DIFaddbegin \DIFadd{\delta ^{2} (x ; x') .
}\DIFaddend \end{equation}
\DIFaddbegin \DIFadd{From \eqref{eqn:holo-sol-implicit}, a recursion can be built, leading to a more explicit form
}\DIFaddend \begin{multline}
	\DIFdelbegin %DIFDELCMD < \label{eqn:Xr-solution}
%DIFDELCMD < 	%%%
\DIFdelend X^{r} (x) = \DIFaddbegin \DIFadd{X^{r} _{0} (x) + }\\ \DIFadd{+ \sum _{k \geq 1} (}\DIFaddend - \DIFdelbegin \DIFdel{4 }\DIFdelend \DIFaddbegin \DIFadd{2)^{k} (}\DIFaddend \mu\DIFdelbegin \DIFdel{\sum _{m \geq 0} \sum _{n \geq 0} \frac{T_{m} (\tau)}{\mu \omega (2 m + 1)} \frac{X_{n} (\sigma)}{\mu k (2 n + 1)} \times }%DIFDELCMD < \\
%DIFDELCMD < 	%%%
\DIFdel{\times }\DIFdelend \DIFaddbegin \DIFadd{)^{2 k -1} }\DIFaddend \int \DIFaddbegin \DIFadd{\prod _{n = 1} ^{k} }\DIFaddend \diff \DIFdelbegin \DIFdel{^2 }\DIFdelend \DIFaddbegin \DIFadd{^{2} }\DIFaddend x\DIFdelbegin \DIFdel{' T_{m} }\DIFdelend \DIFaddbegin \DIFadd{^{(n)} G }\DIFaddend (\DIFdelbegin \DIFdel{\tau '}\DIFdelend \DIFaddbegin \DIFadd{x^{(n - 1)} ; x^{(n)}}\DIFaddend ) \DIFaddbegin \DIFadd{\e ^{\varphi (x^{(n)})} \times }\\ \DIFadd{\times (2 + \mu }\DIFaddend X\DIFdelbegin \DIFdel{_{n} }\DIFdelend \DIFaddbegin \DIFadd{^{r} _{0} }\DIFaddend (\DIFdelbegin \DIFdel{\sigma '}\DIFdelend \DIFaddbegin \DIFadd{x^{(k)}}\DIFaddend )\DIFdelbegin %DIFDELCMD < \left( %%%
\DIFdel{\frac{\omega ^2 \tau ^{\prime 2}}{2} - \frac{k^2 \sigma ^{\prime 2}}{2} }%DIFDELCMD < \right) %%%
\DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{),
}\DIFaddend \end{multline}
\DIFaddbegin \DIFadd{with $x^{(0)} \equiv x$.
}\DIFaddend 
...
\end{equation}
Another candidate for what the vector excitations could relate to is the \DIFaddbegin \DIFadd{$D = 3 + 1$ }\DIFaddend Ashtekar electric field \DIFdelbegin \DIFdel{$\widetilde{E}^{\mu} _{I}$,
}\DIFdelend \DIFaddbegin \DIFadd{$\widetilde{E}^{a} _{i}$,
}\DIFaddend \begin{equation} \label{eqn:AEF-string-vector-excitation}
	\widetilde{E}\DIFdelbegin \DIFdel{^{\mu} _{I} }\DIFdelend \DIFaddbegin \DIFadd{^{a} _{i} }\DIFaddend \sim (\Hat{a}\DIFdelbegin \DIFdel{^{\mu} _{I}}\DIFdelend \DIFaddbegin \DIFadd{^{a} _{i}}\DIFaddend )^{\dagger} \ket{0} .
\end{equation}

\DIFaddbegin \subsection{\DIFadd{Open Questions and Future Directions}}\label{sec:open-questions}

\DIFadd{While our work establishes a promising framework for incorporating LQG insights into string theory, several fundamental questions remain open. Building on the pioneering work of Smolin \mbox{%DIFAUXCMD
\cite{Smolin1998Strings} }\hskip0pt%DIFAUXCMD
and Markopoulou \mbox{%DIFAUXCMD
\cite{Markopoulou1997Causal}}\hskip0pt%DIFAUXCMD
, a more complete framework is needed to show how smooth worldsheet geometries emerge from the discrete, punctured surfaces of spin networks in Loop Quantum Gravity. Our earlier work \mbox{%DIFAUXCMD
\cite{Vaid2017Connecting} }\hskip0pt%DIFAUXCMD
established that the Nambu-Goto action can be viewed as the expectation value of the LQG area operator, but the detailed emergence mechanism requires further development.
}

\DIFadd{The physical interpretation of the emergent holographic dimension $X^r$ also requires further study. Understanding the physical nature of this dimension and its emergence mechanism represents a key theoretical challenge, though the connection to Afshordi's model provides important guidance.
}

\DIFadd{Perhaps most significantly, the relationship between our modified constraint algebra and the fundamental structure of spacetime at the Planck scale requires clarification. Do the two-index Virasoro generators correspond to a deeper geometric principle? Can they be understood by some suitable deformation of the Virasoro algebra to incorporate the cutoff of the sum over Virasoro modes?
}
...
\begin{equation}\DIFdelbegin %DIFDELCMD < \label{eqn:ws-metric}
%DIFDELCMD < 	%%%
\DIFdelend \DIFaddbegin \label{eqn:induced-metric}
	\DIFaddend h_{ab} = \frac{\partial X^\mu}{\partial x^a} \frac{\partial X^\nu}{\partial x^b} G_{\mu\nu},
\end{equation}
...
Following the procedure found in \cite{Grimshaw2010Homogenization}, we start with eq (\ref{eqn:simplified-modified-wave-eqn}) obtained in section \ref{sec:modified-eom}, renaming $\sigma$ as $x$ and dividing by $(F^{+})^2$ such that we have $v^2$ on the second term like the mentioned work, and perform a change of variables \DIFdelbegin \DIFdel{$(t , x) \rightarrow (\tau (t , x) , \sigma (t , x))$}\DIFdelend \DIFaddbegin \DIFadd{$(t, x) \rightarrow (\tau (t, x), \sigma (t, x))$}\DIFaddend , where without loss of generality (one needs just to rescale and/or rotate the coordinates) we set
...
\begin{multline} \label{eqn:new-coordinates-constraints-compact}
	...
	\partial _{t} \sigma = v \partial _{x} \tau \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \DIFdelbegin \DIFdel{\ ,
}\DIFdelend \DIFaddbegin \DIFadd{\,
}\DIFaddend \end{multline}
...
\end{multline}
Next, we introduce \DIFdelbegin \DIFdel{$X^{\mu} = \kappa (\tau , \sigma) W^{\mu} (\tau , \sigma)$ }\DIFdelend \DIFaddbegin \DIFadd{$X^{\mu} = \kappa (\tau, \sigma) W^{\mu} (\tau, \sigma)$ }\DIFaddend such that terms proportional to first derivatives of $W^{\mu}$ cancel out. This is obtained by choosing $\kappa = v^{1/2}$ and concluding that $v$ only depends on $x$. With this, eq (\ref{eqn:modified-wave-eqn-new}) turns into a Klein-Gordon equation
\begin{equation}
...
\end{equation}
and \DIFdelbegin \DIFdel{$\tau (t , x)$ }\DIFdelend \DIFaddbegin \DIFadd{$\tau (t, x)$ }\DIFaddend satisfying constraints (\ref{eqn:new-coordinates-constraints-compact}). This shows that our analysis of the action (\ref{eqn:inverse-metric-Polyakov}) is indeed in agreement with the analysis of the inverse area corrected Nambu-Goto action (\ref{eqn:modified-ng-action}).
\DIFaddbegin 

\section{\DIFadd{Relation between Kalb-Ramond field and Quantum of Area 2-form}} \label{sec:KR-QA}

\DIFadd{To see how the area 2-form $\Delta _{ab}$ is related to the Kalb-Ramond field we have to go back to }\autoref{sec:ws-kalb-ramond} \DIFadd{where we looked at the case of the bimetric string action \eqref{eqn:polyakov-bimetric}. We saw that the bimetric Polyakov action can be expressed, quite obviously, as a bimetric Nambu-Goto action \eqref{eqn:double-ng-action}, which we reproduce below for convenience:
}\begin{equation}
	\DIFadd{S = - T \int \diff ^2 x \sqrt{- g} - T' \int \diff ^2 x \sqrt{- h}.
}\end{equation}
\DIFadd{From here if we make the mapping $T' = - T k / 2$ and $h = \Delta ^2 / g$, we recover the Nambu-Goto area-corrected action \eqref{eqn:modified-ng-action}
}\begin{equation}
	\DIFadd{S = - T \int \diff ^2 x \left( \sqrt{- g} - \frac{1}{2} \frac{k \Delta}{\sqrt{- g}} \right) .
}\end{equation}
\DIFadd{Now, as noted in the main text, the condition $h = \Delta^2/g$ can be achieved if the following holds true \eqref{eqn:h-delta-relation}:
}\begin{equation*}
	\DIFadd{h_{ab} = \Delta_{ac} \Delta_{bd} g^{cd},
}\end{equation*}
\DIFadd{where $g^{cd}$ is the inverse of $g_{ab}$. Now, let us assume that $\Delta_{ab}$ can be written as \eqref{eqn:delta-epsilon-relation}:
}\begin{equation*}
	\DIFadd{\Delta_{ab} = \sqrt{\Delta} \epsilon_{ab}.
}\end{equation*}
\DIFadd{Now, we recall that the inverse of a $2\times 2$ matrix can be written using the total-antisymmetric symbol as:
}\begin{equation}
	\DIFadd{g^{ab} = \frac{1}{2g} \epsilon^{ac} \epsilon^{bd} g_{cd},
}\end{equation}
\DIFadd{where $g = \det{g_{ab}}$. Inserting this identity into \eqref{eqn:h-delta-relation}, and using the expression for the the determinant of a matrix in terms of product of epsilon symbols:
}\begin{equation*}
	\DIFadd{g = \det{g_{ab}} = \onehalf \epsilon^{ac} \epsilon^{bd} g_{cd},
}\end{equation*}
\DIFadd{we find that $h = \Delta^2/g$, where $\Delta = \det{\Delta_{ab}}$. What this tells us is that it is natural to identify $\Delta_{ab}$ with the totally anti-symmetric tensor on the worldsheet. Now a two-dimensional manifold can only have one, upto conformal transformations, anti-symmetric tensor and if we require that this tensor be derived from the embedding of the string into the ambient manifold then it makes sense that one should identify $\Delta_{ab}$ with the pullback to the worldsheet of the bulk Kalb-Ramond field $B_{\mu\nu}$.
}

\DIFadd{For a more formal derivation of this fact we start from the action in \eqref{eqn:area-corrected-K-R-action-v2} which describes the area-corrected string coupled to the Kalb-Ramond field, where the Levi-Civita tensor $\varepsilon ^{a b}$ has been written in terms of the quantum of area 2-form $\Delta _{a b}$ by dividing and multiplying by $\sqrt{\Delta}$. Varying this action \wrt \ $\Delta ^{c d}$ yields
}\begin{multline} \DIFadd{\label{eqn:var-act-AC-KR}
	...
	\DIFadd{- \sqrt{\Delta} \Delta ^{a b} \del _{a} X^{\mu} \del _{b} X^{\nu} B_{\mu \nu} \bigg) - }\\
	\DIFadd{- \bigg( \sqrt{- g} - \frac{1}{2} \frac{k \Delta}{\sqrt{- g}} \bigg) \bigg( \frac{1}{2} \sqrt{\Delta} \Delta _{c d} \Delta ^{a b} \del _{a} X^{\mu} \del _{b} X^{\nu} B_{\mu \nu} + }\\
	\DIFadd{+ \sqrt{\Delta} \delta ^{a} _{c} \delta ^{b} _{d} \del _{a} X^{\mu} \del _{b} X^{\nu} B_{\mu \nu} \bigg) \Bigg) ,
}\end{multline}
\DIFadd{which when required to vanish can be rearranged into
}\begin{multline} \DIFadd{\label{eqn:eqn:var-act-AC-KR-rear}
	\Delta _{c d} \Bigg( \frac{k \sqrt{\Delta}}{2} g^{a b} \del _{a} X^{\mu} \del _{b} X^{\nu} G_{\mu \nu} - }\\
	\DIFadd{- \bigg( g + \frac{3 k \Delta}{4} \bigg) \Delta ^{a b} \del _{a} X^{\mu} \del _{b} X^{\nu} B_{\mu \nu} \Bigg) - }\\
	\DIFadd{- \bigg( g + \frac{k \Delta}{2} \bigg) \del _{c} X^{\mu} \del _{d} X^{\nu} B_{\mu \nu} = 0 .
}\end{multline}
\DIFadd{From here, the quantum of area field $\Delta _{c d}$ can be directly related to the pullback of the Kalb-Ramond 2-form $\del _{c} X^{\mu} \del _{d} X^{\nu} B_{\mu \nu}$ as in
}\begin{equation} \DIFadd{\label{eqn:QA-pullKR}
	\Delta _{c d} = \widetilde{f} \del _{c} X^{\mu} \del _{d} X^{\nu} B_{\mu \nu} ,
}\end{equation}
\DIFadd{where the function $\widetilde{f}$ is given by a contribution from the pullback of the metric and from the Kalb-Ramond field
}\begin{multline} \DIFadd{\label{eqn:1/f-KR}
	\frac{1}{\widetilde{f}} = \frac{1}{g + \dfrac{k \Delta}{2}} \Bigg( \frac{k \Delta}{2} g^{a b} \del _{a} X^{\mu} \del _{b} X^{\nu} G_{\mu \nu} - }\\
	\DIFadd{- \bigg( g + \frac{3 k \Delta}{4} \bigg) \Delta ^{a b} \del _{a} X^{\mu} \del _{b} X^{\nu} B_{\mu \nu} \Bigg) .
}\end{multline}
\DIFadd{When the equation of motion for the metric $g_{a b}$ is satisfied i.e if $g_{a b} = \del _{a} X^{\mu} \del _{b} X^{\nu} G_{\mu \nu}$, then \eqref{eqn:1/f-KR} simplifies to
}\begin{equation} \DIFadd{\label{eqn:1/f-KR-simp}
	\frac{1}{\widetilde{f}} = - \frac{k \sqrt{\Delta}}{g + k \Delta}
}\end{equation}
\DIFadd{or alternatively
}\begin{equation} \DIFadd{\label{eqn:f-KR-simp}
	\widetilde{f} = - \frac{g + k \Delta}{k \sqrt{\Delta}} = - \sqrt{\Delta} \left( \frac{g}{k \Delta} + 1 \right) .
}\end{equation}
\DIFadd{Finally, we can then write the quantum of area 2-form $\Delta _{a b}$ in terms of the pullback of the Kalb-Ramond field $b_{a b} = \del _{a} X^{\mu} \del _{b} X^{\nu} B_{\mu \nu}$ as
}\begin{equation} \DIFadd{\label{eqn:QA-KR-fin}
