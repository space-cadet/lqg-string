...
\AR Following the referee's suggestion we have added the following paragraph right after eq (7.12), relating the cosmological constant $\Lambda$ and the deSitter length $l$:

\emph{Given the observed cosmological constant $\Lambda \approx 1.47 \times 10^{-52} \text{m}^{-2}$ \citep{Collaboration2020Planck}, eq (7.12) implies that $l \approx 1.4 \times 10^{26} \text{m}$. This happens to coincide with the scale of the cosmological horizon \citep{Peebles2003Cosmological}. This would suggest that the scale of the emergent dimension $X^r$ is of the order of the cosmological horizon. Of course, this is a very naive ``back of the envelope'' calculation and would require further investigation to place on a firmer footing.}
...
\AR We agree with the referee's assessment. We have added the following paragraph following the paragraphs discussing the limitations of our ``anomaly free'' claims (which were also added in response to points raised by the referee):

\emph{Additionally, our anomaly-freedom and ghost-freedom analysis is limited to the tree-level constraints; quantum loop corrections and the full quantum effective action may introduce subtleties not captured in this treatment.}

\subsection{General: Writing and Typos}

\RC A full review should be done to the writing to correct typos.

\AR Done. Several typos were found and fixed.
